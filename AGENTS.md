...
- **CI-TAGFAST (2026-08-05).** `.githooks/pre-push` reads the ref lines git feeds it on stdin (`<local ref> <local sha> <remote ref> <remote sha>`, one per pushed ref) and decides `full` vs `skip` **before** invoking `tools/preci.sh`: it runs `tools/preci.sh --full` for every push **except** one specific shape -- a push that is **only** tag ref(s), whose target commit(s) are **already an ancestor of `<remote>/main`** per a live `git ls-remote` (not the local tracking ref -- see the hook's own header, "why ls-remote", for why trusting the tracking ref is not safe here). That case skips the gate entirely (no fast-mode fallback either -- see the header for why fast mode would not add real coverage there). Any branch ref in the same push (`git push --follow-tags`, or `git push origin main v1.2.3` in one invocation), any tag whose commit is **not** yet reachable from `<remote>/main`, and empty/unrecognized stdin all still run `--full` -- the optimization is narrowly scoped to "this exact code already passed the gate once, on the branch push that put it on `main`". Pure decision logic (`classify_push_refs()`/`decide_action()`) is sourceable and unit-tested by `tools/tests/pre_push_gate_test.sh`, independent of any real git remote or a `tools/preci.sh` run.
...
`tools/preci.sh` fast mode detects which layer(s) were actually touched (`origin/main...HEAD` union the working tree) and runs only the matching build+test -- Layer 0 (`make build && make test`) and/or Layer 1 (`glintfx/build-preci`, `GLFW=ON` config, `ctest` under Xvfb). `tools/preci.sh --full` runs the wide net (both glintfx configs, sequentially -- one build in memory at a time -- plus `check_encapsulation.sh`, `tools/check_rml_whitelist.sh` (below), and `gitleaks` if installed). As of CI-TAGFAST (2026-08-05), `.githooks/pre-push` invokes `--full` on every push **except** an only-tag(s) push whose commit(s) already passed it once (see the CI-TAGFAST bullet above) -- so `--full` is no longer purely "occasional manual runs"; it is the default gate on any push that is not a redundant tag. See [`TESTES.md`](TESTES.md#tst-l1-preci).

**`tools/check_rml_whitelist.sh` (`RMLX-0/F4`) -- RmlUi is confined to `glintfx/src/rml/`.** Anyone touching `glintfx/src/` needs to know: including an RmlUi header, or using an RmlUi value type (`Rml::String`/`Vector2`/`Colourb`/`Variant`/`Input`/`Log`), **outside `glintfx/src/rml/` fails the commit** -- 2 frozen exceptions only (`app.cpp`, `system_glfw_dedup.hpp`, the GLFW↔RmlUi bridge). Wired into `tools/preci.sh --full` (above) and `.github/workflows/ci.yml:449`. It carries its own embedded self-test (`tools/check_rml_whitelist.sh --selftest`, two throwaway fixture trees under `mktemp`, never touching the real repo) that proves the gate still has teeth before every real run -- run it by hand after editing the script itself. Full contract, the two censuses behind it, and which wave retires the remaining opaque-pointer debt: [`docs/rmlx-subset.md`](docs/rmlx-subset.md), [ADR-0020](docs/adr/0020-rml-anticorruption-layer.md).
