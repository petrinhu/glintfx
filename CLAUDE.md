...
**Política de CI (canônica desde 2026-07-25):** **todo CI roda no GitHub** -- `.github/workflows/` é o único CI que existe (`ci.yml` é o gate de release; `nightly.yml` é a rede de segurança ASan). **Não existe mais CI local nesta máquina:** o runner Forgejo `claudio` foi desligado e removido em 2026-07-25 e o diretório `.forgejo/` foi apagado do repo -- não recriar, e `forgejo-runner exec` não se aplica mais. **Pendência declarada:** o job pesado (ASan/UBSan + `fonteng`) ficou **temporariamente sem casa**; o líder vai montar um runner self-hosted no GitHub depois e portar a receita salva em `~/.claude/receitas-ci/glintfx-heavy-claudio-forgejo.yml` -- até lá, **não configurar runner self-hosted no GitHub**. Continua valendo: reproduzir falha à mão no container `fedora:42` (`tools/ci/Containerfile.f42` + `tools/ci/build_image.sh`, funcionam sozinhos via `docker build`/`docker run`, sem runner) antes de iterar por push, e rodar o `ctest` de pre-commit local (`tools/preci.sh`). Detalhe operacional completo: [`AGENTS.md`](AGENTS.md#política-de-ci-o-que-roda-onde); histórico da decisão anterior: memória `feedback_ci_policy_canonica`.
