...
> Estado em 2026-07-16: o produto ativo deste repositório é o **glintfx** (lib C++ RmlUi+GL3), lançado e taggeado até **`v0.11.0`** (GitHub, CI verde + nightly sanitizer, suíte de 52 testes GLFW=ON / embed; as 3 releases mais recentes: a **`v0.11.0`** trouxe o **backdrop-ripple** (`L1.22-WAVE`/`L1.22-CAPTURE`): captura de FBO0 + `decorator: ripple(...)` que refrata a UI PRÓPRIA do glintfx em screen-space, custo-zero quando inativo via arm/ensure, embed-only, [ADR-0012](docs/adr/0012-backdrop-capture.md); a **`v0.10.0`** trouxe o **flip suave de fonte** (`L1.20-FONTFLIP`): o motor de fonte próprio clean-room virou o **DEFAULT** (`GLINTFX_OWN_FONT_ENGINE=ON` por padrão), FreeType selecionável em runtime SEM rebuild via o campo `FontEngine font_engine` (`FontEngine::Own` default, `FontEngine::FreeType` pra rollback), [ADR-0011](docs/adr/0011-soft-font-flip.md); a **`v0.9.2`** trouxe suporte a build **Windows** (codepath `#ifdef _WIN32` no loader GL gerado, `wglGetProcAddress`+`opengl32.dll`, cross-compile MinGW). Releases anteriores (histórico em [`CHANGELOG.md`](CHANGELOG.md)): `v0.9.1` fix de UB de teardown do RmlUi + kerning; `v0.9.0` DOM read/write + FORMEV + cobertura de CI; `v0.8.0` loader GL clean-room (`L1.14-GLLOADER`) + focus programático). A Camada 0 (C+ASM puro, zero libc) tem a **implementação COMPLETA**: bootstrap I/O (`_start`, wrappers de syscall, `exit`/`write`/`read`) → harness próprio de teste (`C1`, habilita TDD) → libc-núcleo (memória, string, conversão int↔string) → mini-printf → alocador bump via `mmap`, tudo zero libc e sob TDD + review adversarial (itens em `🔍`, aguardando as ondas `TST-*`/`F1`/`AUD-*`/`REL-TAG` `core-v0.1.0` pro `✅`). A meta plena de internalização clean-room (RmlUi/FreeType/GLFW) segue distante, sem prazo. Leia a seção "glintfx" (produto ativo) e depois a "Camada 0" abaixo.
...
Toolchain glintfx (Camada 1): **CMake >= 3.16**, clang, `glfw-devel`, `freetype-devel`, `mesa-libGL-devel` (Fedora). CI roda sob Xvfb/Mesa llvmpipe (GitHub Actions, o único CI que existe).

**Hospedagem (decisão do líder 2026-07-25):** o projeto vive **só no GitHub** (`https://github.com/petrinhu/glintfx`) -- o Codeberg deixou de aceitar código gerado por LLM/AI e o líder está arquivando o repositório de lá. `origin` e `github` apontam os dois pro GitHub; não existe mais push dual. Nenhum link `codeberg.org` novo em doc.

**Política de CI (canônica desde 2026-07-25):** **todo CI roda no GitHub** -- `.github/workflows/` é o único CI que existe (`ci.yml` é o gate de release; `nightly.yml` é a rede de segurança ASan; `heavy.yml` carrega as pernas `sanitize`/`fonteng`). O runner Forgejo `claudio` foi desligado e removido em 2026-07-25 e o diretório `.forgejo/` foi apagado do repo -- não recriar, e `forgejo-runner exec` não se aplica mais. **O job pesado (ASan/UBSan + `fonteng`) já tem casa:** um runner GitHub self-hosted **conteinerizado e efêmero** (`tools/ci/Containerfile.runner` + unit `systemd --user`), teto de 8 GiB de memória (swap desligado) e 4 CPUs, gatilho só `push` na `main` (filtrado por caminho) + `workflow_dispatch`, **zero `pull_request`** (mitigação de runner self-hosted em repo público) -- receita portada de `~/.claude/receitas-ci/glintfx-heavy-claudio-forgejo.yml`, provada E2E múltiplas vezes (última: run `30368396354`, 2026-07-28, 4/4 verde). Continua valendo: reproduzir falha à mão no container `fedora:42` (`tools/ci/Containerfile.f42` + `tools/ci/build_image.sh`, funcionam sozinhos via `docker build`/`docker run`, sem runner) antes de iterar por push, e rodar o `ctest` de pre-commit local (`tools/preci.sh`). Detalhe operacional completo: [`AGENTS.md`](AGENTS.md#política-de-ci-o-que-roda-onde); histórico da decisão anterior: memória `feedback_ci_policy_canonica`.
