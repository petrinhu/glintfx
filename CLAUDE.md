...
Toolchain glintfx (Camada 1): **CMake >= 3.16**, clang, `glfw-devel`, `freetype-devel`, `mesa-libGL-devel` (Fedora). CI roda sob Xvfb/Mesa llvmpipe (GitHub Actions; mais o `heavy.yml` do Forgejo no runner local `claudio`, container `fedora:42`); validar localmente com `forgejo-runner exec` antes de mexer no workflow do Forgejo.
...
**Política de CI (canônica desde 2026-07-25):** **todo CI roda no GitHub, inclusive os pesados** -- `.github/workflows/ci.yml` é o único gate de release. O runner self-hosted local `claudio` (`.forgejo/workflows/heavy.yml`, `runs-on: docker`, container `fedora:42`, roda ASan+fonteng) fica como **fallback apenas**. Pendência declarada: o líder vai configurar o `claudio` no GitHub depois -- até lá, **não configurar runner self-hosted no GitHub**. Continua valendo: reproduzir falha no container `fedora:42` local (`tools/ci/Containerfile.f42`) antes de iterar por push, e rodar o `ctest` de pre-commit local (`tools/preci.sh`). Detalhe operacional completo: [`AGENTS.md`](AGENTS.md#política-de-ci-o-que-roda-onde); histórico da decisão anterior: memória `feedback_ci_policy_canonica`.
