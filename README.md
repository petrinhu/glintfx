...
- **Physical input state channel and window lifecycle for `App`, v0.19.0 (`HOSTIN-1..5`):** `is_key_down(Key)`/`is_mouse_button_down(int)`/`get_cursor_pos(float&, float&)` read PHYSICAL keyboard/mouse without linking GLFW directly -- closing GusWorld's exactly-one production blocker for migrating to `App` mode -- plus `request_close()`/`set_close_request_callback` (a USER-initiated-close veto only) and `window_focus`/`window_iconify` state. Also lands `LOGTHR-1`, a dedup/throttle for repeated RmlUi warnings (~98.5% log-volume reduction measured live). See [`docs/embed-integration.md`](docs/embed-integration.md) sections 21-22.
...
- **Canal de estado de input físico e ciclo de vida de janela pro `App`, v0.19.0 (`HOSTIN-1..5`):** `is_key_down(Key)`/`is_mouse_button_down(int)`/`get_cursor_pos(float&, float&)` leem teclado/mouse FÍSICOS sem linkar o GLFW direto -- fechando o único bloqueio de produção que o GusWorld reportava pra migrar ao modo `App` -- mais `request_close()`/`set_close_request_callback` (veto só pra close INICIADO PELO USUÁRIO) e estado de `window_focus`/`window_iconify`. Também traz o `LOGTHR-1`, um dedup/throttle pra warnings repetidos do RmlUi (~98,5% de redução de volume de log medida ao vivo). Ver [`docs/embed-integration.md`](docs/embed-integration.md) seções 21-22.
