...
[![Version: 0.14.0](https://img.shields.io/badge/Version-0.14.0-blue.svg)](CHANGELOG.md)
...
> **Current release: v0.14.0** (stable, tagged), 2026-07-21. Full history in [`CHANGELOG.md`](CHANGELOG.md). Battle-tested by a real consumer (GusWorld, an SDL3 game) across the releases below. The **2D game framework** direction ([ADR-0015](docs/adr/0015-framework-2d-atomized-architecture.md)) opened at v0.12.0 (atomized module skeleton + the standalone `App`'s own input); v0.13.0 added the **i18n module** (multi-language: catalogs, plurals, runtime locale switch); v0.14.0 adds the **audio module** (`A3-AUDIO`: sound-effect/music playback via vendored miniaudio, deterministic RAII lifecycle) and **carves the glintfx-authored fx shaders into their own genuinely OFF-able atom** (`FX-CARVE-1`, new `lean-ui` preset) -- both genuinely opt-in atoms, same independence discipline as i18n.

**Delivered (v0.2.x-v0.14.0):**
...
- **Layer 1 = glintfx:** the C++ library documented here. **Released and the repository's active product** (tag `v0.14.0`).
...
> **Lançamento atual: v0.14.0** (estável, taggeada), 2026-07-21. Histórico completo em [`CHANGELOG.md`](CHANGELOG.md). Testada na prática por um consumidor real (GusWorld, um jogo SDL3) ao longo das versões abaixo. A direção **framework de jogos 2D** ([ADR-0015](docs/adr/0015-framework-2d-atomized-architecture.md)) abriu na v0.12.0 (esqueleto de módulos atomizado + input próprio do `App` standalone); a v0.13.0 adicionou o **módulo i18n** (multi-idioma: catálogos, plurais, troca de locale em runtime); a v0.14.0 adiciona o **módulo audio** (`A3-AUDIO`: reprodução de efeito sonoro/música via miniaudio vendorizado, ciclo de vida RAII determinístico) e **recorta os shaders de fx autorais da glintfx no próprio átomo genuinamente OFF-ável** (`FX-CARVE-1`, preset novo `lean-ui`) -- os dois átomos genuinamente opt-in, mesma disciplina de independência do i18n.

**Entregue (v0.2.x-v0.14.0):**
...
- **Camada 1 = glintfx:** a biblioteca C++ documentada aqui. **Lançada e é o produto ativo deste repositório** (tag `v0.14.0`).
