...
- **CI active (GitHub Actions, the single release gate).** The full 99-test suite (GLFW=ON) and 70-test embed suite (GLFW=OFF), plus lint/scan and coverage gates, run on every push/PR via `.github/workflows/ci.yml`. A self-hosted Forgejo runner (`claudio`, `.forgejo/workflows/heavy.yml`) keeps an ASan/LSan/UBSan sanitize gate plus the own-font-engine regression job as a **local fallback** for depth; it is not the release gate.
...
- **CI ativo (GitHub Actions, o único gate de release).** A suíte completa de 99 testes (GLFW=ON) e a suíte embed de 70 testes (GLFW=OFF), mais os gates de lint/scan e cobertura, rodam em todo push/PR via `.github/workflows/ci.yml`. Um runner self-hosted do Forgejo (`claudio`, `.forgejo/workflows/heavy.yml`) mantém um gate ASan/LSan/UBSan mais o job de regressão do motor de fonte próprio como **fallback local** de profundidade; não é o gate de release.
