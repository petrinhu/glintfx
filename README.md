...
- **Modos de janela (só `App`):** `AppConfig::window_mode` + `App::set_window_mode()` -- `Windowed`/`Maximized`/`FullscreenDesktop`/`FullscreenExclusive`, com `Maximized` respeitando o próprio painel/barra de tarefas do window manager (o fix que um consumidor real precisava) e queries vivas (`window_mode()`, `get_window_size()`). Ver [`docs/window-modes.md`](docs/window-modes.md).
