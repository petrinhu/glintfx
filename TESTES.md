...
- **Os checks pesados `sanitize` (ASan/LSan/UBSan) + `fonteng` (`GLINTFX_OWN_FONT_ENGINE=ON`) rodam no `.github/workflows/heavy.yml`, runner GitHub self-hosted conteinerizado e efêmero desde 2026-07-25.** Rodavam antes só no runner Forgejo `claudio`, desligado e removido da máquina junto com o diretório `.forgejo/`; a receita foi portada de `~/.claude/receitas-ci/glintfx-heavy-claudio-forgejo.yml`. Teto de 8 GiB de memória (swap desligado) + 4 CPUs, gatilho só `push` na `main` (filtrado por caminho) + `workflow_dispatch`, zero `pull_request` (mitigação de runner self-hosted em repo público). Provado E2E múltiplas vezes, última run `30368396354` (2026-07-28, 4/4 verde). Reprodução manual sob demanda continua disponível: `tools/ci/build_image.sh` + `docker run` contra a `localhost/glintfx-ci:f42`.
...
| TST-L1-SECRETS | Verificação de secrets (T8) | Credencial/segredo commitado: guardrail barato para um repo **público**, mesmo sem auth por design | `gitleaks` / `trufflehog` no histórico + no pre-CI. Custo ~zero, sempre-verde |
...
| TST-L1-PRECI | Pré-CI local (T15) | Espelhar o gate de CI **antes** do push: gate **rápido** por padrão (só a camada tocada) + modo **`--full`** (rede larga: os 2 configs GLFW ON/OFF + `TST-L1-STATIC`/`SECRETS`) | `tools/preci.sh`, enganchado via `.githooks/pre-push` (opt-in). **Entregue** (2026-07-10) -- ver subseção abaixo |
| TST-L1-MUTSBX | Isolamento de mutation testing / anti-contaminação de build dir (processo, não gate de CI) | Sandbox isolado por sessão pra sabotar uma **cópia** commitada em vez do arquivo rastreado (`mutation_sandbox.sh`) + wrapper de `ctest` que detecta build dir compartilhado contaminado por uma mutação de outro agente, inclusive transitória (`ctest_guarded.sh`) | `tools/mutation_sandbox.sh` (create/run/destroy, `/var/tmp`, blob commitado via `git archive`) + `tools/ctest_guarded.sh` (snapshot de árvore + fingerprint sha256+mtime-ns dos executáveis, antes/depois). **Entregue** (`BUILDDIR-MUTACAO`, W23, 2026-07-31) -- ver subseção abaixo |
...
Gate de cobertura no CI (job `coverage`, **`.github/workflows/ci.yml`, o único workflow que roda cobertura**), config única GLFW=ON (superconjunto da suíte embed-only), demos OFF (biblioteca instrumentada não linka com um demo não-instrumentado — ver comentário `GLINTFX_COVERAGE` em `glintfx/CMakeLists.txt`).
...
- **Recalibração.** Quando a cobertura medida subir de forma sustentada, o piso deve subir junto (mesma fórmula: arredondar para baixo ao múltiplo de 5, menos 5pp) — um piso desatualizado que fica muito abaixo da realidade não protege nada. Recalibrar via `tools/coverage_report.sh` local + atualizar o valor `--floor` em `.github/workflows/ci.yml` no mesmo commit (é o único workflow rodando cobertura).
...
- **Cobertura roda num lugar só: `.github/workflows/ci.yml`.** Existe um único job de cobertura, no GitHub. Não inventar alternativa (matrix, suíte reduzida, retry silencioso) nem duplicar o job noutro workflow sem aval do líder.

#### TST-L1-PRECI

Gate LOCAL do dia a dia, `tools/preci.sh` (SPDX, `set -euo pipefail`, `--help` bilíngue), enganchado via `.githooks/pre-push`. **Decisão do líder (2026-07-10): pre-push, não pre-commit** — a suíte glintfx warm leva ~2min (confirmado na validação: 39/39 testes, ~130s de `ctest` mesmo com build incremental), o que quebraria o fluxo de commits pequenos e frequentes; pre-push é o checkpoint "antes de ficar visível num remoto".

- **Modo rápido (default).** Detecta a(s) camada(s) de fato tocada(s) comparando `origin/main...HEAD` **UNIÃO** a working tree (staged + unstaged + untracked — pega também trabalho não commitado). Camada 0 tocada (`src/**`, `include/**`, `Makefile`, `tests/**` da raiz) → `make build && make test`. glintfx tocado (`glintfx/**`) → configure incremental em `glintfx/build-preci` (config única `GLINTFX_BACKEND_GLFW=ON`, superconjunto) + `xvfb-run -a ctest --output-on-failure`. Se ambos, roda os dois. Se nada relevante mudou, passa rápido com aviso — não builda à toa. Se `origin/main` não existe localmente (clone novo/offline), cai para diff contra a árvore vazia em vez de falhar duro.
- **Modo `--full`.** A rede larga: `make build && make test` (Camada 0, incondicional, custo desprezível) + glintfx nas **2 configs** (`GLINTFX_BACKEND_GLFW` ON e OFF, builds **sequenciais** — guardrail de OOM, só um build glintfx na memória por vez) + `tools/check_encapsulation.sh` (sub-check do `TST-L1-STATIC`) + `gitleaks` (`TST-L1-SECRETS`) se instalado. `gitleaks` ausente = aviso, **não** falha o gate (não bloquear por ferramenta opcional não instalada). Pensado para rodadas ocasionais (ex.: antes de taggear release), não a cada push.
- **Ativação (opt-in, por clone):** `git config core.hooksPath .githooks`. Sem essa config, o git ignora `.githooks/` (só olha `.git/hooks/` por padrão) e o hook não roda. Bypass pontual: `git push --no-verify` (usar com moderação).
- **Validado em 2026-07-10** na árvore real: modo rápido detectou corretamente Camada 0 tocada (dummy em `src/`, `make build && make test` rodou, todos os testes passaram) e Camada 1 tocada (dummy em `glintfx/`, build fresco puxando RmlUi via FetchContent, 39/39 testes ctest passaram, ~130s warm numa segunda rodada incremental — confirma a estimativa de ~2min que motivou a decisão pre-push). Modo "nada tocado" passa rápido sem buildar nada.

#### TST-L1-MUTSBX

Origem: `BUILDDIR-MUTACAO` (W23, `TODO.md`), nascida de um incidente real de 2026-07-30 — 21 coredumps fantasma de `capture_framebuffer_smoke` numa janela fechada de 6 minutos, causados pelo build dir compartilhado entre a mutação de um agente e a suíte completa de outro. Relato completo do incidente, as duas invocações canônicas verificadas ao vivo, os limites declarados e o histórico das três rodadas de review adversarial de cada ferramenta: [`AGENTS.md`](AGENTS.md#mutation-testing-sandbox-sabotage-a-copy-never-the-shared-build-dir-builddir-mutacao-w23) (não duplicado aqui).

- **`tools/mutation_sandbox.sh create/run/destroy`** — isola cada sessão de mutation testing (a mutação **adversarial de review**, não uma métrica de suíte — ver a emenda da linha "Mutation testing" em "Fora de escopo" abaixo) num build dir próprio em `/var/tmp`, materializado de um sha **commitado** via `git archive`. Sabota-se a cópia; o arquivo rastreado nunca é tocado.
- **`tools/ctest_guarded.sh <build-dir> [ctest-args]`** — wrapper de `ctest` (mesmo isolamento Xvfb/`XDG_RUNTIME_DIR` do `run_layer1_config()` do `tools/preci.sh`) que invalida a rodada inteira, **inclusive os testes verdes**, se a árvore rastreada de `glintfx/` OU o fingerprint (sha256+mtime-ns) dos executáveis de teste mudarem entre o antes e o depois — pega até mutação **transitória** (aplicada e revertida dentro da própria janela guardada, o padrão exato do incidente).
- **Limites declarados** (não é cobertura total): o guard de árvore vigia só `glintfx/` — a Camada 0 (`src/`, `include/`) não é coberta; o fingerprint cobre só os binários que a seleção `-R`/`-E`/`-L` daquela invocação exercita, não o build dir inteiro; nenhuma das duas ferramentas vê mutação de arquivo untracked.
- Nem `mutation_sandbox.sh` nem `ctest_guarded.sh` são gate automático de CI — são ferramentas de **processo**, usadas por quem estiver fazendo review adversarial que executa (mutation testing manual, não pontuação de suíte).
...
- **Mutation testing como MÉTRICA automatizada de suíte** (score de CI, gate por-mutante) — segue vaidade cara neste porte: a suíte é majoritariamente render-statistics (difícil mutar com sinal real). Reavaliar em módulos de **lógica pura** (ex.: `DataBinder`) se crescerem. **Isto não significa que mutação não acontece aqui** — lido cru, o bullet acima sugeria isso, e o incidente `BUILDDIR-MUTACAO` (W23) desmentiu: mutação **adversarial de review** (sabotar deliberadamente uma peça pra provar que a suíte/guard pega, não pontuar a suíte inteira) é praticada nesta casa e desde 2026-07-31 tem protocolo mecanizado — `tools/mutation_sandbox.sh` + `tools/ctest_guarded.sh`, ver `TST-L1-MUTSBX` acima.
