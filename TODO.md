...
> **Estado:** v1 ENTREGUE e mergeada (tag glintfx `v0.1.0`) — ondas LW1–LW4 ✅. Pendências pós-v1 (drenadas da INBOX): LW5 = bugfix/hardening rápido paralelizável; LW6 = CI + DX de distribuição; LW7 = backends adiados; LW8 = internalização clean-room (endgame). Caminho recomendado: fechar LW5 (barato) → LW6 (gate de CI verde) antes de abrir a v2.
...
- **Doc de moved-from** (F1 review): anotar em `App` e `UiLayer` que métodos exceto `ok()`/dtor têm UB em objeto movido (convenção do projeto, igual `unique_ptr`).
- **`UiEvent::Type` default** (F1 review): `UiEvent{}` cai em `MouseMove`; considerar `Type::None` para default mais seguro.
- **`GlStateGuard` cobertura** (F1 review, F1-M2): salva só a textura da unidade ativa (unit 0); não varre todas as texture units nem `READ_FRAMEBUFFER` — coberto pela cláusula "invariantes do host" do ADR-0008(d); revisitar se algum efeito da F2 usar mais units.
...
- **GAP-4 `set_focus(id)` no `UiLayer` (feature p/ v0.2.4/v2 — semente, pedido GusWorld 2026-06-30, NÃO urgente):** expor `set_focus(const char* id)`/`focus_element(id)` para hosts cujo MODELO é dono da seleção (ex.: GusWorld, canon 1B) — assim o anel de foco do glintfx espelha a seleção do host em vez de Tab/setas moverem o foco sozinhos. **Contornado** no GusWorld deixando o foco glintfx inerte (sem `:focus`) e dirigindo o destaque 100% por data-binding (`data-class-sel`). Boa candidata pra v2 (componentes focáveis).
- **`version()` no build embed-only (v0.2.4 review, ADIADO):** `version()` vive em `app.hpp`, só incluído com `GLINTFX_BACKEND_GLFW=ON` — consumidores embed-only (ex.: GusWorld) NÃO têm `glintfx::version()`. Mover para um header sempre-disponível (mudança de superfície pública, two-way-door).
- **UA-stylesheet: recompile redundante por load (v0.2.4 T1 review, Minor):** quando o documento tem `<link>`/`<style>` próprio, a stylesheet original é compilada no `LoadDocument` e depois descartada quando o `Bootstrap::load()` a substitui pela mesclada — pequeno desperdício constante por load; inerente à mesclagem via API pública pós-load.
- **Gate de encapsulamento: unificar a lista da regex (v0.2.4 review):** padronizar o grep include-based com a lista completa `(Rml|glfw|gl3w|GL/|stb|SDL)` (docs já usam; formalizar se virar gate de CI).
