...
> **Estado:** v1 ENTREGUE e mergeada (glintfx `v0.1.0` → `v0.2.4`, 6 tags) — ondas LW1–LW4 ✅. **LW5 e LW6 FECHADAS, trio CI-now da LW-TST FECHADO** (2026-07-02): CI verde nos 2 remotos com job novo `lint-and-scan` (estático+secrets+deps, 18/8 testes de build+test) + `find_package` verificado ponta-a-ponta + `L1.1-ERRSTRAT` ratificado (`bool ok()` definitivo) + `version()` exposta no embed-only + `UiEvent::Type::None`. Pendências pós-v1: **LW-TST/LW-AUD restante** = `TST-L1-DECODE/GLLEAK/CONTRACT/PRECI` + `AUD-L1-*` (5 itens), pull-based, sem gatilho de urgência hoje (`TST-L1-MEM` fechado 2026-07-02 com nightly ASan/UBSan/LSan; 2 achados do `AUD-L1-PARSE` já corrigidos adiantados); **LW7** = backends adiados; **LW8** = internalização clean-room (endgame).
>
> **Ordem recomendada (Kanban, finish-before-start):** LW5/LW6/trio-CI/`TST-L1-MEM` fechados. Próxima frente é **pull-based**: aguardar o próximo achado do GusWorld que justifique `TST-L1-DECODE`/`GLLEAK`/`CONTRACT`, ou puxar `AUD-L1-MEM` (agora desbloqueado -- consome o run sanitizado do `TST-L1-MEM`). **LW7/LW8 = keep-back.** A **v2** roda em modo **pull incremental** (backlog, não agendada — ver seção v2).
