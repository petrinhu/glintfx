...
| `load_font(const char* path)` (`glintfx/include/glintfx/draw2d.hpp:948`) | `Font2d` | TX1 -- parses an in-file SFNT/TrueType face via the sovereign C core. `ok() == false` on null path, open failure, the 64 MiB blob cap, a parse failure, or an out-of-memory condition during parse -- never a crash, `noexcept` (FONT-NOTHROW, W22). Lazy per-size atlas. See "Text" above. |
...
| `load_font(const char* path)` (`glintfx/include/glintfx/draw2d.hpp:948`) | `Font2d` | TX1 -- parseia uma face SFNT/TrueType em arquivo via o núcleo C soberano. `ok() == false` em path nulo, falha ao abrir, o teto de 64 MiB de blob, falha de parse, ou uma condição de esgotamento de memória durante o parse -- nunca crash, `noexcept` (FONT-NOTHROW, W22). Atlas por-tamanho preguiçoso. Ver "Texto" acima. |
