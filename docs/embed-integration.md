...
1. When `cfg.load_gl` is `true` (the default): `glx_gl_load()` (`ui_layer.cpp:125`) resolves glintfx's GL function-pointer table against **whatever GL context is current at that exact instant** -- there is no wait, no retry, no deferred resolution.
2. `impl_->engine.attach(&impl_->clock, impl_->w, impl_->h, cfg.font_engine)` (`ui_layer.cpp:128`) -- the SAME `RenderGl3::init()`-then-`Bootstrap::init()` sequence described for `App` above, with the SAME shader-compile-on-construction dependency on a populated function table.
...
3. `UiLayerConfig::load_gl = false` is an opt-out for a host that has ALREADY called `glx_gl_load()` itself (e.g. a second `UiLayer` instance sharing a context already loaded by the first, `ui_layer.cpp:70-75` documents the reload as idempotent/cheap either way) -- it skips step 1 but still requires the SAME "context already current" precondition for step 2's shader compile.
...
1. Quando `cfg.load_gl` é `true` (o padrão): `glx_gl_load()` (`ui_layer.cpp:125`) resolve a tabela de ponteiros de função GL da glintfx contra **o que quer que seja o contexto GL corrente naquele instante exato** -- não há espera, nem retry, nem resolução adiada.
2. `impl_->engine.attach(&impl_->clock, impl_->w, impl_->h, cfg.font_engine)` (`ui_layer.cpp:128`) -- a MESMA sequência `RenderGl3::init()`-depois-`Bootstrap::init()` descrita para o `App` acima, com a MESMA dependência de compilação-de-shader-na-construção contra uma tabela de função populada.
...
3. `UiLayerConfig::load_gl = false` é um opt-out para um host que JÁ chamou `glx_gl_load()` ele mesmo (ex.: uma segunda instância de `UiLayer` compartilhando um contexto já carregado pela primeira, `ui_layer.cpp:70-75` documenta o reload como idempotente/barato de qualquer forma) -- pula o passo 1 mas ainda exige a MESMA pré-condição "contexto já corrente" para a compilação de shader do passo 2.
