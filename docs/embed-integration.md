...
**EN:** `glintfx::GlStateGuard` (`glintfx/src/gl_state.hpp:20-63`) is taken around every `render()` (`engine.cpp:78-83`). It snapshots and restores exactly the following:

**PT:** O `glintfx::GlStateGuard` (`glintfx/src/gl_state.hpp:20-63`) é tomado em volta de cada `render()` (`engine.cpp:78-83`). Ele salva e restaura exatamente o seguinte:
...
**EN:**
- glintfx uses its own **GL 3.3 core-profile loader** (`glintfx/src/gl_loader.h`/`.c`, generated from the public Khronos `gl.xml` registry, L1.14-GLLOADER; replaces the previously vendored gl3w) via `RMLUI_GL3_CUSTOM_LOADER=<gl_loader.h>` (`glintfx/CMakeLists.txt:1508`) and links the `glloader` INTERFACE target (`:203`). MSAA is disabled (`RMLUI_NUM_MSAA_SAMPLES=0`).
- `UiLayerConfig.assume_gl_loaded` (`ui_layer.hpp`, default `false`; canonical since `SEED-LOADGL-NOME`, 2026-08-04) and its deprecated predecessor `UiLayerConfig.load_gl` (default `true`, kept functional for one version): the loader runs iff `load_gl && !assume_gl_loaded`. When it runs, the constructor calls `glx_gl_load()` against the host current context, loading glintfx's own GL function pointer table (`ui_layer.cpp`). ⚠️ **Inverted polarity**: `load_gl = false` and `assume_gl_loaded = true` mean the SAME thing ("skip my own load, the table is already populated") -- `assume_gl_loaded = false` is NOT the migration of `load_gl = false`, it is its opposite.
...
- **Recommendation:** a host with its own loader keeps `assume_gl_loaded = false` (the default). glintfx loads its own GL function pointers; the host keeps its loader for its own renderer. Set `assume_gl_loaded = true` only if the host also uses glintfx's loader and already called `glx_gl_load()`. Precondition: the GL context must be current before constructing the `UiLayer` (`glx_gl_load` needs a current context). ⚠️ **Measured, not theoretical (`GLPROC-CRASH`/class-wide sweep, 2026-07-30):** there is NO scenario where asking to skip the loader is simultaneously safe AND useful -- as the FIRST glintfx entity in a process it fails clean (`ok() == false`) but never builds a working `UiLayer`; after another entity already ran the loader it works, but saves only one cheap, idempotent call over leaving the default. See `UiLayerConfig`'s own doc-comment (`ui_layer.hpp`) and `glintfx::gl_proc_address()`'s own doc-comment (`gl_proc.hpp`) for the full measurement.
- **`UiLayerConfig.gl_proc_resolver` (`GLLOADER-HOST`, 2026-08-04) -- give glintfx YOUR resolver.** `GlProcResolver = void* (*)(const char* name)`, matching `SDL_GL_GetProcAddress`/glad/GLEW's own signature exactly (no `void* user` -- a stateful resolver keeps its state as a host-side `static`). This is what makes `assume_gl_loaded = true` actually SAFE for a host whose GL context comes from a loader glintfx's own glX/EGL/dlsym chain does not cover -- SDL, ANGLE, a host's own EGL wrapper, an engine embedding glad/GLEW -- populations for which `assume_gl_loaded = true` used to just leave the table empty (`GLPROC-CRASH`, above). Rule: when the load path is active (`load_gl && !assume_gl_loaded`) and a resolver is set, it -- **exclusively, no fallback** -- populates glintfx's table (`glx_gl_load_with()`, `gl_loader.h`/`.c`); a missing core symbol fails clean, same contract as `glx_gl_load()` (`ok() == false`, logged, never a crash). When the load path is SKIPPED (`assume_gl_loaded = true`, or the deprecated `load_gl = false`) but a resolver was ALSO set, that is a contradiction -- the skip claim wins, the resolver is never called, a warning names why. `resolver == nullptr` (the default): byte-for-byte unchanged behaviour. **Out of scope:** `glintfx::App` (owns its own GLFW window/loader already, no resolver to inject) and the "no instance yet" lazy-populate case (`CAPTURE-FREE-LOADER`, W27, TODO.md -- a related but DIFFERENT fronteira: glintfx populating its OWN table with its OWN chain, with no instance in sight).

**PT:**
- O glintfx usa seu **próprio loader GL 3.3 core profile** (`glintfx/src/gl_loader.h`/`.c`, gerado a partir do registro público Khronos `gl.xml`, L1.14-GLLOADER; substitui o gl3w antes vendorizado) via `RMLUI_GL3_CUSTOM_LOADER=<gl_loader.h>` (`glintfx/CMakeLists.txt:1508`) e linka o target INTERFACE `glloader` (`:203`). MSAA desligado (`RMLUI_NUM_MSAA_SAMPLES=0`).
- `UiLayerConfig.assume_gl_loaded` (`ui_layer.hpp`, default `false`; canônico desde `SEED-LOADGL-NOME`, 2026-08-04) e o predecessor deprecated `UiLayerConfig.load_gl` (default `true`, mantido funcional por uma versão): o loader roda sse `load_gl && !assume_gl_loaded`. Quando roda, o construtor chama `glx_gl_load()` contra o contexto corrente do host, carregando a tabela de ponteiros GL do próprio glintfx (`ui_layer.cpp`). ⚠️ **Polaridade invertida**: `load_gl = false` e `assume_gl_loaded = true` significam a MESMA coisa ("pule meu próprio load, a tabela já está populada") -- `assume_gl_loaded = false` NÃO é a migração de `load_gl = false`, é o oposto dela.
...
- **Recomendação:** um host com loader próprio mantém `assume_gl_loaded = false` (o default). O glintfx carrega os próprios ponteiros de função GL; o host mantém o loader dele para o renderer dele. Use `assume_gl_loaded = true` só se o host também usar o loader do glintfx e já tiver chamado `glx_gl_load()`. Pré condição: o contexto GL deve estar corrente antes de construir o `UiLayer` (o `glx_gl_load` exige contexto corrente). ⚠️ **Medido, não teórico (`GLPROC-CRASH`/varredura de classe, 2026-07-30):** NÃO existe cenário onde pedir pra pular o loader é ao mesmo tempo seguro E útil -- como PRIMEIRA entidade glintfx de um processo falha limpo (`ok() == false`) mas nunca constrói um `UiLayer` funcional; depois de outra entidade já ter rodado o loader funciona, mas economiza só uma chamada idempotente e barata sobre deixar o padrão. Ver o próprio doc-comment de `UiLayerConfig` (`ui_layer.hpp`) e o próprio doc-comment de `glintfx::gl_proc_address()` (`gl_proc.hpp`) pra medição completa.
- **`UiLayerConfig.gl_proc_resolver` (`GLLOADER-HOST`, 2026-08-04) -- entregue à glintfx o SEU resolvedor.** `GlProcResolver = void* (*)(const char* name)`, casando com a própria assinatura de `SDL_GL_GetProcAddress`/glad/GLEW exatamente (sem `void* user` -- um resolvedor com estado carrega esse estado como `static` do lado do host). É isto que torna `assume_gl_loaded = true` de fato SEGURO pra um host cujo contexto GL vem de um loader que a própria cadeia glX/EGL/dlsym da glintfx não cobre -- SDL, ANGLE, um wrapper EGL próprio do host, um motor que embarca glad/GLEW -- populações pra quem `assume_gl_loaded = true` antes só deixava a tabela vazia (`GLPROC-CRASH`, acima). Regra: quando o caminho de load está ativo (`load_gl && !assume_gl_loaded`) e um resolvedor está definido, é ele -- **exclusivamente, sem fallback** -- que popula a tabela da glintfx (`glx_gl_load_with()`, `gl_loader.h`/`.c`); um símbolo core ausente falha de forma limpa, mesmo contrato do `glx_gl_load()` (`ok() == false`, logado, nunca um crash). Quando o caminho de load está SENDO PULADO (`assume_gl_loaded = true`, ou o `load_gl = false` deprecated) mas um resolvedor TAMBÉM foi definido, isso é uma contradição -- a alegação de pular vence, o resolvedor nunca é chamado, um aviso nomeia o motivo. `resolver == nullptr` (o default): comportamento inalterado byte a byte. **Fora de escopo:** `glintfx::App` (já é dono da própria janela/loader GLFW, sem resolvedor pra injetar) e o caso "ainda sem instância" de populamento preguiçoso (`CAPTURE-FREE-LOADER`, W27, TODO.md -- uma fronteira relacionada mas DIFERENTE: a glintfx populando a PRÓPRIA tabela com a PRÓPRIA cadeia, sem instância à vista).
...
**Alignment overload (SCROLL-ALIGN, W26).** `scroll_element_into_view(id, align_with_top)` always re-anchors to an edge, even when the element is already fully visible -- confirmed by a consumer-side controlled experiment (GusWorld, 2026-08-01): scrolling an already-visible, already-selected list item still jumped the scroll container by dozens of pixels (`y=204 -> y=136` measured). A second overload, `scroll_element_into_view(id, ScrollAlign align)` (`glintfx/include/glintfx/scroll_types.hpp`), exposes RmlUi's own `Rml::ScrollAlignment` enum -- **vertical axis only** -- so a caller can request `ScrollAlign::Adaptive`: a delta-zero no-op when the element is already fully in view, centers it otherwise. The `bool` overload is **unchanged and NOT deprecated** (existing callers keep compiling as-is); the new overload has **no default argument** on purpose -- giving it one would make `scroll_element_into_view("id")` ambiguous between the two overloads and break every existing single-argument call site with a hard compile error. `ScrollAlign::Start`/`End` reproduce the `bool` overload's `true`/`false` semantics byte-for-byte (same `Rml::ScrollAlignment` mapping under the hood); `Center`/`Nearest` are additionally exposed for completeness. Horizontal alignment, animated (`ScrollBehavior::Smooth`) scrolling, and `ScrollParentage::Closest` are **not** exposed by this overload -- RmlUi's own defaults apply (Nearest/Instant/All, the same defaults the `bool` overload already used), a future extension point if a consumer needs them. Verified by `scroll_align_sanity` (embed, both configs) and an `app_scroll_smoke` parity smoke for `App`.
...
**Sobrecarga de alinhamento (SCROLL-ALIGN, W26).** `scroll_element_into_view(id, align_with_top)` sempre reancora a uma borda, mesmo quando o elemento já está totalmente visível -- confirmado por um experimento controlado do lado do consumidor (GusWorld, 2026-08-01): rolar um item de lista já visível e já selecionado ainda pulava o container de rolagem em dezenas de pixels (`y=204 -> y=136` medido). Uma segunda sobrecarga, `scroll_element_into_view(id, ScrollAlign align)` (`glintfx/include/glintfx/scroll_types.hpp`), expõe o próprio enum `Rml::ScrollAlignment` do RmlUi -- **só eixo vertical** -- para que um chamador possa pedir `ScrollAlign::Adaptive`: um no-op de delta-zero quando o elemento já está totalmente na área visível, centraliza senão. A sobrecarga `bool` está **inalterada e NÃO é depreciada** (chamadores existentes continuam compilando como estão); a sobrecarga nova tem **sem argumento default** de propósito -- dar um a ela tornaria `scroll_element_into_view("id")` ambíguo entre as duas sobrecargas e quebraria todo call-site de um único argumento existente com erro de compilação forte. `ScrollAlign::Start`/`End` reproduzem a semântica `true`/`false` da sobrecarga `bool` byte a byte (o mesmo mapeamento de `Rml::ScrollAlignment` por baixo dos panos); `Center`/`Nearest` são adicionalmente expostos por completude. Alinhamento horizontal, rolagem animada (`ScrollBehavior::Smooth`), e `ScrollParentage::Closest` **não** são expostos por esta sobrecarga -- os defaults do próprio RmlUi valem (Nearest/Instant/All, os mesmos defaults que a sobrecarga `bool` já usava), um ponto de extensão futuro se algum consumidor precisar. Verificado por `scroll_align_sanity` (embed, nos 2 configs) e um smoke de paridade em `app_scroll_smoke` pro `App`.
...
**EN:** `App::set_frame_callback(std::function<void(float dt_seconds)> cb)` (`glintfx/include/glintfx/app.hpp:568`) fires once per `render()`/`run()` iteration, with the GL context current, AFTER the frame clear and BEFORE the UI render pass -- see `Engine::render_standalone`'s doc-comment (`glintfx/src/engine.hpp:108-143`) for the exact insertion point and the full GL-state save/restore contract. This section adds one thing that comment does not cover: **hosting a complete, independent GL renderer inside `cb` is a supported, validated pattern**, not just a place for a couple of draw calls.
...
**State restore -- already documented, referenced here, not duplicated.** The hook may leave arbitrary GL state behind (bind any program/VAO/texture, toggle blend/depth/scissor/cull/stencil, rebind the draw framebuffer); it is automatically restored to the RmlUi-established baseline before the UI render pass runs. Full mechanics and the RmlUi-source citation backing them: `glintfx/src/engine.hpp:108-143` ("GL STATE CONTRACT" in `render_standalone`'s doc-comment). Do not re-derive this here -- read that comment directly if the exact restore set matters to your renderer (e.g. which texture unit, which blend equation).

**PT:** `App::set_frame_callback(std::function<void(float dt_seconds)> cb)` (`glintfx/include/glintfx/app.hpp:568`) dispara uma vez por iteração de `render()`/`run()`, com o contexto GL corrente, APÓS o clear do frame e ANTES do passe de render de UI -- ver o doc-comment de `Engine::render_standalone` (`glintfx/src/engine.hpp:108-143`) para o ponto de inserção exato e o contrato completo de salvar/restaurar estado GL. Esta seção acrescenta uma coisa que aquele comentário não cobre: **hospedar um renderer GL completo e independente dentro de `cb` é um padrão suportado e validado**, não só um lugar pra algumas chamadas de desenho.
...
**Restauração de estado -- já documentada, referenciada aqui, não duplicada.** O hook pode deixar estado GL arbitrário para trás (vincular qualquer programa/VAO/textura, alternar blend/depth/scissor/cull/stencil, revincular o draw framebuffer); ele é automaticamente restaurado à base estabelecida pelo RmlUi antes do passe de render de UI rodar. Mecânica completa e a citação de source do RmlUi por trás dela: `glintfx/src/engine.hpp:108-143` ("GL STATE CONTRACT" no doc-comment de `render_standalone`). Não re-derive isto aqui -- leia aquele comentário diretamente se o conjunto exato de restauração importar pro seu renderer (ex.: qual unidade de textura, qual equação de blend).

## 22. Physical input state + window lifecycle (`App`-only, `HOSTIN-1..5`, v0.19.0; `SEED-SCANCODE`, W27) / Estado de input físico + ciclo de vida da janela (só `App`, `HOSTIN-1..5`, v0.19.0; `SEED-SCANCODE`, W27)
...
### Physical input state channel (`HOSTIN-1`/`HOSTIN-2`; `SEED-SCANCODE`)

| Method | Returns | Notes |
| :--- | :--- | :--- |
| `is_key_down(Key k)` (`app.hpp:405`) | `bool` | Level state -- true while the physical key stays held. GLFW's own auto-repeat does not re-set it (already true since the initial press). `false` when `!ok()` or `k` is out of range. |
| `is_scancode_down(int scancode)` (`app.hpp:432`) | `bool` | RAW-scancode PARALLEL channel (`SEED-SCANCODE`, W27) -- reaches a physical key `Key` cannot name at all (`GLFW_KEY_UNKNOWN`, e.g. ABNT2's extra `/`, scancode 97). `scancode` in `[0, 511]`; out-of-range (including negative) returns `false`. Physical only -- see the ABNT2 bullet below. |
| `is_mouse_button_down(int button)` (`app.hpp:448`) | `bool` | `button` in `[0, 7]` -- GLFW's own honest ceiling (`GLFW_MOUSE_BUTTON_LAST == 7`), **not** the UI route's 3-button contract (unchanged). Out-of-range (including negative) returns `false`. |
| `get_cursor_pos(float& x, float& y)` (`app.hpp:456`) | `void` | Framebuffer physical pixels, top-left origin, y-down -- same space as `UiEvent::MouseMove`/`get_element_box`. `(0, 0)` when `!ok()` or before any cursor-position event. |
| `set_key_callback(fn(Key key, KeyAction action, int modifiers))` (`app.hpp:469`) | `void` | Edge-detected: fires once per physical Press/Release, once per OS auto-repeat tick (`KeyAction::Repeat`). Fires from inside `poll_events()` (copy-before-invoke). Null/empty `cb` is a safe no-op. |

**Key vocabulary.** `enum class Key` (`glintfx/include/glintfx/ui_event.hpp:32`) was extended APPEND-ONLY (the pre-existing `None`..`PageDown` entries keep their exact numeric values, machine-enforced by `static_assert`s right below the enum) with ~92 new entries mirroring GLFW's own nameable key set: `A`-`Z`, `Digit0`-`Digit9`, `F1`-`F12`, both-side modifiers (`LeftShift`/`RightShift`/...), lock keys, punctuation, `World1`/`World2`, and the numpad (`Kp0`-`Kp9`, `KpDecimal`, ...), ending with a `Count` sentinel that is never a real key. `KeyAction` (`ui_event.hpp:261`) is `{ Press, Release, Repeat }`.
...
- **ABNT2 keyboard gap -- CLOSED by `SEED-SCANCODE` (W27).** GLFW's key tokens are PHYSICAL-POSITION tokens named after the US layout -- correct semantics for WASD-style movement, layout-agnostic by construction. Consequence for ABNT2: the `ç` key arrives as the physical `Semicolon` position (position semantics, not label semantics); the second `/` key (right of the right Shift, scancode 97) has **no GLFW token at all** -- a known upstream gap, `GLFW_KEY_UNKNOWN`. GLFW's own key callback still delivers the raw `scancode` for such a key regardless (confirmed against GLFW's own `glfwSetKeyCallback` doc-comment, `glfw3.h`); `is_scancode_down(97)` polls it directly, no `Key` enumerator required. `World1`/`World2` ARE covered by the enum (the ISO extra key some ABNT2/European boards report).
- **Synthetic vs. physical channels stay separate.** `App::process_event()` (the synthetic/replay/test channel, unchanged since A1) does NOT write the physical state table read by `is_key_down`/`is_scancode_down`/`is_mouse_button_down`/`get_cursor_pos` -- a replay harness cannot forge a "held key" a real game would read for its own movement. `UiEvent` has no scancode field at all, by design (`SEED-SCANCODE` deliberately did not extend it).

### Window lifecycle (`HOSTIN-3`/`HOSTIN-4`/`HOSTIN-5`)

| Method | Returns | Notes |
| :--- | :--- | :--- |
| `request_close()` (`app.hpp:488`) | `void` | Programmatic quit -- BYPASSES the veto below entirely (an intentional act by the code that owns the loop, e.g. a confirmed "Sair" menu item; routing it through the veto would deadlock a confirmation dialog against its own confirm action). |
| `set_close_request_callback(fn() -> bool)` (`app.hpp:501`) | `void` | Veto for a USER-initiated close (X button, Alt+F4) only -- `false` keeps the window open (e.g. show "save before exit?"); any other outcome, or no callback registered, lets the close proceed (matches pre-`HOSTIN-4` behaviour). NEVER fires for `request_close()`. |
| `set_window_focus_callback(fn(bool focused))` / `is_window_focused()` (`app.hpp:520`/`522`) | `void` / `bool` | WINDOW-level focus (OS has-input-focus-at-all), prefixed `window_` so it never collides with the pre-existing DOM `set_focus_callback`. Typical use: auto-pause/duck audio on alt-tab. |
| `set_window_iconify_callback(fn(bool iconified))` / `is_window_iconified()` (`app.hpp:521`/`523`) | `void` / `bool` | Minimize observation, same prefix rationale. Fail-high (`false`) when `!ok()`. |
...
### Canal de estado de input físico (`HOSTIN-1`/`HOSTIN-2`; `SEED-SCANCODE`)

| Método | Retorna | Notas |
| :--- | :--- | :--- |
| `is_key_down(Key k)` (`app.hpp:405`) | `bool` | Estado de nível -- true enquanto a tecla física continuar segurada. O próprio auto-repeat do GLFW não re-seta (já era true desde o pressionar inicial). `false` quando `!ok()` ou `k` fora de faixa. |
| `is_scancode_down(int scancode)` (`app.hpp:432`) | `bool` | Canal PARALELO de scancode cru (`SEED-SCANCODE`, W27) -- alcança uma tecla física que o `Key` não consegue nomear de jeito nenhum (`GLFW_KEY_UNKNOWN`, ex.: o `/` extra do ABNT2, scancode 97). `scancode` em `[0, 511]`; fora de faixa (inclusive negativo) retorna `false`. Só físico -- ver o item ABNT2 abaixo. |
| `is_mouse_button_down(int button)` (`app.hpp:448`) | `bool` | `button` em `[0, 7]` -- o próprio teto honesto do GLFW (`GLFW_MOUSE_BUTTON_LAST == 7`), **não** o contrato de 3 botões da rota de UI (inalterado). Fora de faixa (inclusive negativo) retorna `false`. |
| `get_cursor_pos(float& x, float& y)` (`app.hpp:456`) | `void` | Pixels físicos de framebuffer, origem superior-esquerda, y pra baixo -- mesmo espaço de `UiEvent::MouseMove`/`get_element_box`. `(0, 0)` quando `!ok()` ou antes de qualquer evento de posição de cursor. |
| `set_key_callback(fn(Key key, KeyAction action, int modifiers))` (`app.hpp:469`) | `void` | Edge-detectado: dispara uma vez por Press/Release físico, uma vez por tick de auto-repeat do SO (`KeyAction::Repeat`). Dispara de dentro de `poll_events()` (cópia-antes-de-invocar). `cb` nulo/vazio é no-op seguro. |

**Vocabulário de tecla.** `enum class Key` (`glintfx/include/glintfx/ui_event.hpp:32`) foi estendido APPEND-ONLY (as entradas pré-existentes `None`..`PageDown` mantêm os valores numéricos exatos, reforçado por máquina via `static_assert`s logo abaixo do enum) com ~92 entradas novas espelhando o próprio conjunto nomeável de teclas do GLFW: `A`-`Z`, `Digit0`-`Digit9`, `F1`-`F12`, modificadores dos dois lados (`LeftShift`/`RightShift`/...), teclas de trava, pontuação, `World1`/`World2`, e o numpad (`Kp0`-`Kp9`, `KpDecimal`, ...), terminando num sentinela `Count` que nunca é uma tecla de verdade. `KeyAction` (`ui_event.hpp:261`) é `{ Press, Release, Repeat }`.
...
- **Lacuna do teclado ABNT2 -- FECHADA pela `SEED-SCANCODE` (W27).** Os tokens de tecla do GLFW são tokens de POSIÇÃO física, nomeados pelo layout US -- semântica certa pra movimento estilo WASD, agnóstica de layout por construção. Consequência no ABNT2: a tecla `ç` chega como a posição física `Semicolon` (semântica de posição, não de rótulo); a segunda tecla `/` (à direita do Shift direito, scancode 97) **não tem token GLFW nenhum** -- uma lacuna upstream conhecida, `GLFW_KEY_UNKNOWN`. O próprio callback de tecla do GLFW entrega o `scancode` cru dessa tecla do mesmo jeito (confirmado contra o próprio doc-comment de `glfwSetKeyCallback`, `glfw3.h`); `is_scancode_down(97)` consulta ele direto, sem precisar de enumerador `Key` nenhum. `World1`/`World2` SÃO cobertas pelo enum (a tecla extra ISO que alguns teclados ABNT2/europeus reportam).
- **Canais sintético e físico permanecem separados.** `App::process_event()` (o canal sintético/replay/teste, inalterado desde o A1) NÃO escreve a tabela de estado física lida por `is_key_down`/`is_scancode_down`/`is_mouse_button_down`/`get_cursor_pos` -- um harness de replay não pode forjar uma "tecla segurada" que um jogo real leria pro próprio movimento. `UiEvent` não tem campo scancode nenhum, por desenho (a `SEED-SCANCODE` deliberadamente não o estendeu).

### Ciclo de vida da janela (`HOSTIN-3`/`HOSTIN-4`/`HOSTIN-5`)

| Método | Retorna | Notas |
| :--- | :--- | :--- |
| `request_close()` (`app.hpp:488`) | `void` | Saída programática -- BYPASSA o veto abaixo inteiramente (um ato intencional do código dono do loop, ex.: um item de menu "Sair" confirmado; roteá-lo pelo veto travaria um diálogo de confirmação contra a própria ação de confirmar). |
| `set_close_request_callback(fn() -> bool)` (`app.hpp:501`) | `void` | Veto só pra close INICIADO PELO USUÁRIO (botão X, Alt+F4) -- `false` mantém a janela aberta (ex.: mostrar "salvar antes de sair?"); qualquer outro resultado, ou nenhum callback registrado, deixa o close prosseguir (bate com o comportamento pré-`HOSTIN-4`). NUNCA dispara pra `request_close()`. |
| `set_window_focus_callback(fn(bool focused))` / `is_window_focused()` (`app.hpp:520`/`522`) | `void` / `bool` | Focus de nível JANELA (o SO ter foco de input de todo), prefixado `window_` pra nunca colidir com o `set_focus_callback` de DOM pré-existente. Uso típico: auto-pausar/abafar áudio no alt-tab. |
| `set_window_iconify_callback(fn(bool iconified))` / `is_window_iconified()` (`app.hpp:521`/`523`) | `void` / `bool` | Observação de minimizar, mesmo racional de prefixo. Fail-high (`false`) quando `!ok()`. |
...
## 24. Programmatic font-face registration (`load_font_face`, `UI-FONTFACE`, v0.24.0) / Registro programático de face de fonte (`load_font_face`, `UI-FONTFACE`, v0.24.0)

**EN:** `UiLayer::load_font_face(const FontFaceDesc&)` / `App::load_font_face(const FontFaceDesc&)` (`glintfx/include/glintfx/font_face.hpp`) register a font face at RUNTIME, from C++, without authoring an RCSS `@font-face` block inside a `.rml` document -- the use case is a host that only decides which font file to load AFTER some other subsystem resolves a localisation pack, a DLC, or a user preference. Both forward to the same `Engine::load_font_face` -> `Bootstrap::load_font_face` implementation, the ONE place in glintfx that ever calls the underlying `Rml::LoadFontFace` free function.

- **`FontFaceDesc` is a struct, not positional parameters** (same precedent as `AppConfig`/`UiLayerConfig`/`AudioConfig`/`GamepadsConfig`) -- its field set can grow across releases without an ABI/source break at every call site:
  ```cpp
  struct FontFaceDesc {
    const char* path = nullptr;      // required
    const char* family = nullptr;    // nullptr = derive (engine-dependent, see below)
    FontStyle   style  = FontStyle::Normal;   // Normal | Italic
    FontWeight  weight = FontWeight::Auto;    // Auto | Normal | Bold
    bool        fallback_face = false;
  };
  ```
- **No document/`load()` ordering constraint.** Font registration is process-wide RmlUi state (`Rml::LoadFontFace` has no `Context`/document parameter at all) -- call `load_font_face` before or after `load()`, it only requires a successful `ok()`.
- **Guards (`AUD-TEC-5` fail-high):** `false`, no crash, no `Rml::LoadFontFace` call made, when not `ok()`, or when `desc.path` is `nullptr`/`""`. Beyond that, the return value IS RmlUi's own `Rml::LoadFontFace` result, unchanged -- a malformed/non-font file, or one the `FileInterface` cannot open (honours `set_asset_base_url`, same as every other asset load), surfaces as `false` exactly like RmlUi itself reports it.
- **`family = nullptr` is ENGINE-DEPENDENT -- read this before relying on it.** With no explicit family, `load_font_face` forwards to `Rml::LoadFontFace`'s family-less overload, and the two engines selectable via `glintfx::FontEngine` (section 17) derive the registered family DIFFERENTLY:
  - `FontEngine::Own` derives the family from the font FILE'S OWN STEM, lower-cased (`assets/OpenSans-Regular.ttf` -> `"opensans-regular"`) -- it does not parse the SFNT `'name'` table.
  - `FontEngine::FreeType` reads the font file's own SFNT `'name'` table instead (the SAME file typically registers as `"Open Sans"`, a designer-chosen string with no fixed relationship to the file's name on disk).

  A caller that ships with either engine (`FontEngine::FreeType` is the documented rollback, section 17) and needs a PREDICTABLE family string MUST pass an explicit, non-null `family` -- doing so drives BOTH engines through the family-WITH-parameters overload, which registers that EXACT string on either engine identically.
- **`fallback_face = true` unlocks the SAME per-glyph fallback mechanism RCSS's `-rmlui-fallback-face: true;` `@font-face` property drives** (`FontEngineOwn::fallback_faces_`/`BakeGlyph()`, `src/font_engine_own.cpp`), now reachable without authoring a document just to register a fallback face -- RmlUi walks every fallback-registered face in REGISTRATION ORDER, process-wide, when a span's primary font is missing a glyph.
- **Testing:** `tests/fontface_load_sanity.cpp` (registered in `tests/CMakeLists.txt` under `if(GLINTFX_OWN_FONT_ENGINE)`, both `GLINTFX_BACKEND_GLFW` sides, drives everything through `UiLayer`) -- seven criteria: valid registration+render, hostile input (`nullptr`/`""`/nonexistent/non-font path), a two-engine A/B run proving an explicit `family` resolves the asymmetry above, the `family=nullptr` derivation asymmetry itself (documented, not forced equal between engines), programmatic `fallback_face=true`, white-box before-`init()`/not-yet-`attach()`ed guards on `Bootstrap`/`Engine` directly, and a `style`/`weight` mapping check (two same-family faces differing on exactly one axis each, resolved via `FontEngineOwn::FindBestFace`'s own scoring). The `App`-SPECIFIC forwarding wire (a separate method body, `src/app.cpp`, that happens to share the same `Engine::load_font_face` underneath) is covered by a second file, `tests/app_font_face_sanity.cpp` (`if(GLINTFX_BACKEND_GLFW)` only) -- an adversarial review found `App::load_font_face` completely unexercised by the `UiLayer`-only suite (a mutation short-circuiting it to `false` still passed 99/100 of the tracked tests) before this second file existed.

**PT:** `UiLayer::load_font_face(const FontFaceDesc&)` / `App::load_font_face(const FontFaceDesc&)` (`glintfx/include/glintfx/font_face.hpp`) registram uma face de fonte em RUNTIME, a partir de C++, sem autorar um bloco RCSS `@font-face` dentro de um documento `.rml` -- o caso de uso é um host que só decide qual arquivo de fonte carregar DEPOIS de algum outro subsistema resolver um pacote de localização, uma DLC, ou uma preferência de usuário. Ambos encaminham à mesma implementação `Engine::load_font_face` -> `Bootstrap::load_font_face`, o ÚNICO lugar na glintfx que algum dia chama a função livre `Rml::LoadFontFace` subjacente.

- **`FontFaceDesc` é uma struct, não parâmetros posicionais** (mesmo precedente de `AppConfig`/`UiLayerConfig`/`AudioConfig`/`GamepadsConfig`) -- o conjunto de campos dela cresce entre releases sem quebra de ABI/fonte em cada call site (ver o bloco de código acima).
- **Sem restrição de ordem vs. documento/`load()`.** Registro de fonte é estado do RmlUi em nível de PROCESSO (`Rml::LoadFontFace` não tem parâmetro nenhum de `Context`/documento) -- chame `load_font_face` antes ou depois de `load()`, só exige um `ok()` bem-sucedido.
- **Guards (fail-high `AUD-TEC-5`):** `false`, sem crash, sem chamada nenhuma a `Rml::LoadFontFace`, quando não `ok()`, ou quando `desc.path` é `nullptr`/`""`. Além disso, o valor de retorno É o próprio resultado de `Rml::LoadFontFace` do RmlUi, inalterado -- um arquivo malformado/não-fonte, ou um que a `FileInterface` não consegue abrir (honra `set_asset_base_url`, igual a todo outro carregamento de asset), surge como `false` exatamente como o próprio RmlUi reporta.
- **`family = nullptr` é DEPENDENTE-DE-MOTOR -- leia isto antes de depender disso.** Sem família explícita, `load_font_face` encaminha à sobrecarga sem-família de `Rml::LoadFontFace`, e os dois motores selecionáveis via `glintfx::FontEngine` (seção 17) derivam a família registrada de jeitos DIFERENTES:
  - `FontEngine::Own` deriva a família do próprio STEM do arquivo de fonte, minusculizado (`assets/OpenSans-Regular.ttf` -> `"opensans-regular"`) -- não parseia a tabela `'name'` SFNT.
  - `FontEngine::FreeType` lê a própria tabela `'name'` SFNT do arquivo de fonte em vez disso (o MESMO arquivo tipicamente registra como `"Open Sans"`, uma string escolhida pelo designer, sem relação fixa com o nome do arquivo em disco).

  Um chamador que sai com qualquer um dos dois motores (`FontEngine::FreeType` é o rollback documentado, seção 17) e precisa de uma string de família PREVISÍVEL PRECISA passar um `family` explícito e não-nulo -- fazer isso dirige OS DOIS motores pela sobrecarga com-parâmetros-de-família, que registra essa string EXATA em qualquer um dos motores, identicamente.
...
## 26. GL context lifecycle and window-creation order (`App` vs `UiLayer`) (`DOC-GLLIFE`) / Ciclo de vida do contexto GL e ordem de criação de janela (`App` vs `UiLayer`) (`DOC-GLLIFE`)

**EN:** glintfx has two entirely different regimes for who creates the window and the GL context (ADR-0008): `App` owns both (standalone); `UiLayer` attaches to a context the HOST already owns (embed). Getting the order wrong in embed mode is a common integration trap for a new consumer -- this section makes the exact, code-verified sequence explicit for both.

**PT:** A glintfx tem dois regimes inteiramente diferentes para quem cria a janela e o contexto GL (ADR-0008): o `App` possui os dois (standalone); o `UiLayer` se anexa a um contexto que o HOST já possui (embed). Errar a ordem em modo embed é uma armadilha comum de integração para um consumidor novo -- esta seção deixa explícita a sequência exata, verificada no código, para os dois.

### `App`: the framework owns the whole sequence / `App`: o framework é dono da sequência inteira

**EN:** Inside `App::App(AppConfig cfg)` (`glintfx/src/app.cpp:117-224`), in this fixed order:

1. **`window.create(cfg.title, cfg.width, cfg.height, initial_mode)`** (`WindowGlfw::create`, `glintfx/src/window_glfw.cpp:55-222`) -- validates `window_mode` first (an invalid enum falls back to `Windowed`, `app.cpp:128-138`), then requests the window: `glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3)`, `GLFW_CONTEXT_VERSION_MINOR, 3)`, `GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE)` (`window_glfw.cpp:84-86`) -- **a GL 3.3 core-profile context is what glintfx always requests; `AppConfig` has no field to change this.** MSAA is disabled process-wide via `RMLUI_NUM_MSAA_SAMPLES=0` (`glintfx/CMakeLists.txt:1416`, already covered in section 3). Then `glfwCreateWindow(...)` (`window_glfw.cpp:176`) and `glfwMakeContextCurrent(win_)` (`window_glfw.cpp:185`) -- the context becomes current, on the calling thread, before anything else in `create()` runs -- and finally `glx_gl_load()` (`window_glfw.cpp:186`), glintfx's own GL 3.3 loader, resolving its function-pointer table against that now-current context. `create()` returns `false` if either `glfwCreateWindow` or `glx_gl_load()` fails (`window_glfw.cpp:177`, `186-190`), and `App::ok()` reflects this all the way up (`app.cpp:139`: `if (!impl_->window.create(...)) return;` -- the constructor returns early, `impl_->ok` stays `false`).
2. **`SystemInterfaceGlfwDedup` constructed** against `window.handle()` (`app.cpp:142`).
3. **`Engine::attach(...)`** (`app.cpp:179` -> `Engine::attach`, `glintfx/src/engine.cpp:89-99`): `RenderGl3::init()` runs FIRST (`engine.cpp:92`) -- it constructs `RenderInterface_GL3`, whose constructor immediately issues GL calls to compile the built-in shaders (`render_gl3.cpp:451-477`; upstream ctor at `examples/RmlUi/Backends/RmlUi_Renderer_GL3.cpp:777-782`). **This is exactly why step 1's ordering matters:** the shader compiler needs both a current context AND a populated function-pointer table, and step 1 is the only place either is established. THEN `Bootstrap::init(...)` runs (`engine.cpp:92` -> `bootstrap.cpp:930` onward): `Rml::SetSystemInterface`/`SetRenderInterface` (`bootstrap.cpp:935-936`) -> optionally `SetFontEngineInterface` (`:994-995`, `L1.20-FONTFLIP`) -> `SetFileInterface` (`:1030`) -> `Rml::Initialise()` (`:1032`) -> (fx module only) construct+register the 3 decorator instancers, which themselves require `Rml::Initialise()` to have already populated the style-sheet parser registry (`:1086-1119`, see the long ordering comment on `Impl::polygon_instancer`, `:99-118`) -> `Rml::CreateContext("main", ...)` (`:1139`).

**Net effect: the host (the game) has zero GL setup responsibility with `App`.** Window, context, and loader are entirely internal to step 1, in a fixed, non-reorderable sequence with no hook point for a caller to intervene between them.

**PT:** Dentro de `App::App(AppConfig cfg)` (`glintfx/src/app.cpp:117-224`), nesta ordem fixa:

1. **`window.create(cfg.title, cfg.width, cfg.height, initial_mode)`** (`WindowGlfw::create`, `glintfx/src/window_glfw.cpp:55-222`) -- valida `window_mode` primeiro (um enum inválido cai pra `Windowed`, `app.cpp:128-138`), depois pede a janela: `glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3)`, `GLFW_CONTEXT_VERSION_MINOR, 3)`, `GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE)` (`window_glfw.cpp:84-86`) -- **um contexto GL 3.3 core-profile é o que a glintfx sempre pede; `AppConfig` não tem campo para mudar isto.** MSAA é desabilitado em nível de processo via `RMLUI_NUM_MSAA_SAMPLES=0` (`glintfx/CMakeLists.txt:1416`, já coberto na seção 3). Depois `glfwCreateWindow(...)` (`window_glfw.cpp:176`) e `glfwMakeContextCurrent(win_)` (`window_glfw.cpp:185`) -- o contexto fica corrente, na thread chamadora, antes de qualquer outra coisa em `create()` rodar -- e por fim `glx_gl_load()` (`window_glfw.cpp:186`), o próprio loader GL 3.3 da glintfx, resolvendo a tabela de ponteiros de função contra esse contexto agora corrente. `create()` retorna `false` se `glfwCreateWindow` OU `glx_gl_load()` falhar (`window_glfw.cpp:177`, `186-190`), e `App::ok()` reflete isso até o topo (`app.cpp:139`: `if (!impl_->window.create(...)) return;` -- o construtor retorna cedo, `impl_->ok` fica `false`).
2. **`SystemInterfaceGlfwDedup` construído** contra `window.handle()` (`app.cpp:142`).
3. **`Engine::attach(...)`** (`app.cpp:179` -> `Engine::attach`, `glintfx/src/engine.cpp:89-99`): `RenderGl3::init()` roda PRIMEIRO (`engine.cpp:92`) -- constrói `RenderInterface_GL3`, cujo construtor emite chamadas GL imediatamente para compilar os shaders embutidos (`render_gl3.cpp:451-477`; ctor upstream em `examples/RmlUi/Backends/RmlUi_Renderer_GL3.cpp:777-782`). **É exatamente por isto que a ordem do passo 1 importa:** o compilador de shader precisa tanto de um contexto corrente QUANTO de uma tabela de ponteiros de função populada, e o passo 1 é o único lugar onde qualquer um dos dois é estabelecido. DEPOIS `Bootstrap::init(...)` roda (`engine.cpp:92` -> `bootstrap.cpp:930` em diante): `Rml::SetSystemInterface`/`SetRenderInterface` (`bootstrap.cpp:935-936`) -> opcionalmente `SetFontEngineInterface` (`:994-995`, `L1.20-FONTFLIP`) -> `SetFileInterface` (`:1030`) -> `Rml::Initialise()` (`:1032`) -> (só módulo fx) constrói+registra os 3 instancers de decorator, que por si só exigem que `Rml::Initialise()` já tenha populado o registro de parsers de stylesheet (`:1086-1119`, ver o comentário longo de ordenação em `Impl::polygon_instancer`, `:99-118`) -> `Rml::CreateContext("main", ...)` (`:1139`).

**Efeito líquido: o host (o jogo) tem responsabilidade ZERO de setup GL com o `App`.** Janela, contexto e loader são inteiramente internos ao passo 1, numa sequência fixa e não-reordenável, sem ponto de gancho para um chamador intervir entre eles.

### `UiLayer`: the host owns the context, glintfx only attaches / `UiLayer`: o host é dono do contexto, a glintfx só se anexa

**EN:** The precondition is already stated in the header: *"Requires a CURRENT GL context owned by the host"* (`glintfx/include/glintfx/ui_layer.hpp:76-78`), and section 2/3 above already say the host must make its context current before construction. `DOC-GLLIFE` makes the MECHANISM behind that precondition explicit. `UiLayer::UiLayer(Config cfg)` (`glintfx/src/ui_layer.cpp:64-88`), in order:

1. When the combined guard `cfg.load_gl && !cfg.assume_gl_loaded` is `true` (the default, since both fields default to "run the loader" -- `assume_gl_loaded` renamed from `load_gl` by `SEED-LOADGL-NOME`, 2026-08-04, see `UiLayerConfig`'s own doc-comment, `ui_layer.hpp`, for the inverted-polarity contract): `glx_gl_load()` (`ui_layer.cpp`) resolves glintfx's GL function-pointer table against **whatever GL context is current at that exact instant** -- there is no wait, no retry, no deferred resolution.
2. `impl_->engine.attach(&impl_->clock, impl_->w, impl_->h, cfg.font_engine)` (`ui_layer.cpp`) -- the SAME `RenderGl3::init()`-then-`Bootstrap::init()` sequence described for `App` above, with the SAME shader-compile-on-construction dependency on a populated function table.

**What happens if the order is inverted** (a host constructs `UiLayer` before creating its own GL context, or without making it current): on Linux, `glx_gl_load()`'s symbol resolution goes through `glXGetProcAddressARB`/`dlsym` against `libGL.so.1` (`glintfx/src/gl_loader.c:906-919`), which on Mesa and most drivers does **not** itself require a current context to hand back a non-null function pointer -- so `glx_gl_load()` can report success (`missing == 0`, `gl_loader.c:841-847`) even with no context current at all. The failure, if it happens, surfaces one level further in, inside `RenderInterface_GL3`'s own constructor (vendored RmlUi code, `RmlUi_Renderer_GL3.cpp`), when it invokes those resolved-but-context-less pointers to compile shaders. **glintfx adds no guard of its own against this specific ordering mistake** -- the header precondition (`ui_layer.hpp:76-78`) is the entire load-bearing contract, and a host that violates it will not necessarily get a clean, observable `ok() == false` the way a missing `libGL.so` would produce (`missing > 0` from `glx_gl_load()` itself); the failure mode downstream of "context resolved a pointer but was never current" is not glintfx's own code to guarantee. **Test this ordering explicitly** if a host's own context creation is conditional or asynchronous relative to `UiLayer` construction.
3. `UiLayerConfig::assume_gl_loaded = true` (or its deprecated predecessor `load_gl = false`) is an opt-out for a host that has ALREADY called `glx_gl_load()` itself (e.g. a second `UiLayer` instance sharing a context already loaded by the first, `ui_layer.cpp:70-75` documents the reload as idempotent/cheap either way) -- it skips step 1 but still requires the SAME "context already current" precondition for step 2's shader compile. ⚠️ Measured to have NO scenario where it is simultaneously safe and useful (`GLPROC-CRASH`, 2026-07-30) -- see the "GL loader" section above.

**PT:** A pré-condição já está declarada no header: *"Exige contexto GL CORRENTE do host"* (`glintfx/include/glintfx/ui_layer.hpp:76-78`), e as seções 2/3 acima já dizem que o host deve tornar o contexto corrente antes da construção. O `DOC-GLLIFE` deixa explícito o MECANISMO por trás dessa pré-condição. `UiLayer::UiLayer(Config cfg)` (`glintfx/src/ui_layer.cpp:64-88`), em ordem:

1. Quando a guarda combinada `cfg.load_gl && !cfg.assume_gl_loaded` é `true` (o padrão, já que os dois campos têm default "rode o loader" -- `assume_gl_loaded` renomeado de `load_gl` pelo `SEED-LOADGL-NOME`, 2026-08-04, ver o próprio doc-comment de `UiLayerConfig`, `ui_layer.hpp`, pro contrato de polaridade invertida): `glx_gl_load()` (`ui_layer.cpp`) resolve a tabela de ponteiros de função GL da glintfx contra **o que quer que seja o contexto GL corrente naquele instante exato** -- não há espera, nem retry, nem resolução adiada.
2. `impl_->engine.attach(&impl_->clock, impl_->w, impl_->h, cfg.font_engine)` (`ui_layer.cpp`) -- a MESMA sequência `RenderGl3::init()`-depois-`Bootstrap::init()` descrita para o `App` acima, com a MESMA dependência de compilação-de-shader-na-construção contra uma tabela de função populada.

**O que acontece se a ordem for invertida** (um host constrói o `UiLayer` antes de criar o próprio contexto GL, ou sem torná-lo corrente): no Linux, a resolução de símbolo do `glx_gl_load()` passa por `glXGetProcAddressARB`/`dlsym` contra `libGL.so.1` (`glintfx/src/gl_loader.c:906-919`), que no Mesa e na maioria dos drivers NÃO exige por si só um contexto corrente para devolver um ponteiro de função não-nulo -- então `glx_gl_load()` PODE reportar sucesso (`missing == 0`, `gl_loader.c:841-847`) mesmo sem nenhum contexto corrente. A falha, se acontecer, surge um nível mais adiante, dentro do próprio construtor de `RenderInterface_GL3` (código vendorizado do RmlUi, `RmlUi_Renderer_GL3.cpp`), quando ele invoca esses ponteiros resolvidos-mas-sem-contexto para compilar shaders. **A glintfx não acrescenta guarda própria contra este erro específico de ordenação** -- a pré-condição do header (`ui_layer.hpp:76-78`) é o contrato inteiro que sustenta isto, e um host que a viola não necessariamente recebe um `ok() == false` limpo e observável do jeito que uma `libGL.so` faltando produziria (`missing > 0` do próprio `glx_gl_load()`); o modo de falha a jusante de "contexto resolveu um ponteiro mas nunca esteve corrente" não é código da própria glintfx a garantir. **Teste esta ordenação explicitamente** se a criação de contexto do próprio host for condicional ou assíncrona em relação à construção do `UiLayer`.
3. `UiLayerConfig::assume_gl_loaded = true` (ou o predecessor deprecated `load_gl = false`) é um opt-out para um host que JÁ chamou `glx_gl_load()` ele mesmo (ex.: uma segunda instância de `UiLayer` compartilhando um contexto já carregado pela primeira, `ui_layer.cpp:70-75` documenta o reload como idempotente/barato de qualquer forma) -- pula o passo 1 mas ainda exige a MESMA pré-condição "contexto já corrente" para a compilação de shader do passo 2. ⚠️ Medido sem NENHUM cenário onde é ao mesmo tempo seguro e útil (`GLPROC-CRASH`, 2026-07-30) -- ver a seção "GL loader" acima.

### Runtime window-mode transitions do not re-trigger creation / Transições de modo de janela em runtime não re-disparam a criação

**EN:** `App::set_window_mode`/`WindowGlfw::set_mode` (section on window modes, `docs/window-modes.md`) reuses the SAME GLFW window and context across a mode switch: `glfwSetWindowMonitor(...)` (`window_glfw.cpp:336`, `:357`, `:384`, `:388`) is the only window-system call `set_mode()` makes -- there is no `glfwCreateWindow`, no `glfwMakeContextCurrent`, no `glx_gl_load()` call anywhere in `WindowGlfw::set_mode()` (`window_glfw.cpp:313-403`, read in full). A runtime fullscreen/maximize/windowed toggle (e.g. an options-menu Alt+Enter) therefore never re-triggers the creation sequence above -- the GL context and its loaded function table stay the SAME ones `App::App()`'s original `create()` call produced, for the entire life of the `App` instance.

**PT:** `App::set_window_mode`/`WindowGlfw::set_mode` (seção de modos de janela, `docs/window-modes.md`) reutiliza a MESMA janela e contexto GLFW através de uma troca de modo: `glfwSetWindowMonitor(...)` (`window_glfw.cpp:336`, `:357`, `:384`, `:388`) é a única chamada ao sistema de janelas que `set_mode()` faz -- não há `glfwCreateWindow`, nem `glfwMakeContextCurrent`, nem chamada a `glx_gl_load()` em lugar nenhum de `WindowGlfw::set_mode()` (`window_glfw.cpp:313-403`, lido por inteiro). Um toggle de fullscreen/maximizar/windowed em runtime (ex.: Alt+Enter num menu de opções) portanto nunca re-dispara a sequência de criação acima -- o contexto GL e a tabela de função carregada continuam sendo os MESMOS que a chamada `create()` original de `App::App()` produziu, pela vida inteira da instância `App`.

### Scope note / Nota de escopo

**EN:** This section documents the mechanism this repository's own code implements (cited above, line-by-line). The specific ad-hoc probes the consumer wrote to map this behaviour empirically (`poc_gl_single_context`, `msaa_glintfx_probe`) live in the consumer's own repository, not in glintfx -- their specific findings could not be cross-checked against this repo's source and are not reproduced here.

**PT:** Esta seção documenta o mecanismo que o próprio código deste repositório implementa (citado acima, linha a linha). As probes ad-hoc específicas que o consumidor escreveu para mapear este comportamento empiricamente (`poc_gl_single_context`, `msaa_glintfx_probe`) vivem no repositório do próprio consumidor, não na glintfx -- os achados específicos delas não puderam ser cruzados contra o source deste repo e não são reproduzidos aqui.

## 27. App-mode raw-GL cohabitation contract (`DOC-GLCOHAB`) / Contrato de coabitação GL cru no modo App (`DOC-GLCOHAB`)

**EN:** Section 21 already documents that `set_frame_callback`'s hook is a supported place to host a complete, independent GL renderer. This section answers the three questions a consumer migrating its own window-owning shell onto `glintfx::App` needs answered from the CODE, not discovered by trial and error: (a) is the GL context guaranteed current inside the hook, (b) how does a host resolve a GL function pointer without SDL/GLFW of its own, and (c) exactly what GL state -- INCLUDING which framebuffer -- is bound at hook entry and guaranteed at hook exit. Nothing below overrides section 21's own state-restore description; it makes one thing that description does not spell out explicit: the FRAMEBUFFER identity.

**(a) Yes, guaranteed current -- single-context invariant, cited.** `WindowGlfw::create()` calls `glfwMakeContextCurrent(win_)` exactly once (`window_glfw.cpp:185`), immediately after `glfwCreateWindow`, and `WindowGlfw::set_swap_interval`'s own doc-comment states the invariant this relies on explicitly: *"this class never switches it away for the rest of the App's lifetime"* (`window_glfw.hpp:120-126`). No other call in this codebase re-targets the context to a different window/thread for the life of an `App` instance (section 26 above walks the full construction sequence this invariant is part of). `App::set_frame_callback`'s hook (`app.hpp:568`) fires from inside `Engine::render_standalone` (`engine.cpp:191-209`, called by `App::render()`), which always runs on the SAME thread `App`'s constructor ran on -- so the context that `create()` made current is, unconditionally, still the current one when the hook runs.

**(b) `glintfx::gl_proc_address(const char* name)`** (`glintfx/include/glintfx/gl_proc.hpp`, App-mode only, same `GLINTFX_BACKEND_GLFW` gate as `app.hpp`) -- the `SDL_GL_GetProcAddress` equivalent. A one-line delegation (`glintfx/src/gl_proc.cpp`) to `glx_gl_get_proc_address()`, a NEW single-symbol entry point on the existing private loader (`glintfx/src/gl_loader.h`/`.c`) that reuses the SAME `glX -> EGL -> dlsym` / `wglGetProcAddress -> GetProcAddress` resolution chain `glx_gl_load()` itself already uses per table entry -- zero duplicated resolution logic. Deliberately does NOT expose `glx_gl_load()` itself (the batch entry point) or the internal function-pointer table: both stay private, so this call is the ONE seam that crosses the loader's own `glx_`-prefix boundary (ADR-0013), resolving exactly the symbol a caller names and nothing else. `nullptr`/`""` `name`, and a call made before any `App`/`UiLayer` has run `glx_gl_load()` at least once in-process, both return `nullptr` -- documented fail-high, never a crash. **A made-up name does NOT reliably return `nullptr`, though -- measured, not assumed:** under this library's own Mesa/llvmpipe CI environment, `gl_proc_address("totally_bogus_symbol")` returned a NON-NULL pointer. This is `GLX_ARB_get_proc_address`'s own documented behaviour (an implementation MAY hand back a non-NULL pointer for a name it does not recognise; the caller is expected to have already confirmed the extension/function exists before calling the returned pointer), not a glintfx bug -- the same caveat that applies to calling `glXGetProcAddressARB`/`SDL_GL_GetProcAddress` directly. Only pass a name you already know should exist for the GL 3.3 core-profile context glintfx requests, or a core/EXT/ARB extension separately confirmed present (`glx_gl_get_proc_address`'s own doc-comment, `gl_loader.h`, has the full per-branch reasoning).

**(c) GL state at hook entry/exit -- and the framebuffer-identity correction.** Section 21/`engine.hpp`'s own "GL STATE CONTRACT" note (`Engine::render_standalone`, `engine.cpp:191-209`) already says the hook may leave arbitrary state behind and gets restored to "what RmlUi's own `BeginFrame()` had just set" via a `GlStateGuard` snapshot/restore (`gl_state.hpp:63-158`) taken immediately before the hook runs and applied immediately after. What that baseline concretely IS, confirmed by reading the pinned RmlUi source (`RenderInterface_GL3::BeginFrame`, `examples/RmlUi/Backends/RmlUi_Renderer_GL3.cpp:813-890`, commit `2cd28864ae25ed345b70598751703a5433b12356` per `glintfx/CMakeLists.txt`'s `FetchContent_Declare(RmlUi ...)`): viewport `(0, 0, w, h)`; `GL_SCISSOR_TEST`/`GL_CULL_FACE`/`GL_DEPTH_TEST` disabled; `GL_STENCIL_TEST` enabled (`GL_ALWAYS`, ref `1`, mask all-ones, ops `GL_KEEP`); `GL_BLEND` enabled with the premultiplied-alpha function (`glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA)`, equation `GL_FUNC_ADD`); active texture unit `GL_TEXTURE0`; no shader program bound.

**The draw framebuffer bound at hook entry is NOT FBO 0** -- worth stating loudly, because `Engine`'s own top-of-class comment ("clear + UI + alpha-fix **on FBO0**", `engine.hpp:4`) could otherwise read as a promise it is not quite: `RenderInterface_GL3::BeginFrame()` unconditionally pushes and binds an INTERNAL offscreen render layer right before returning (`RenderLayerStack::PushLayer`/`BeginFrame`, `examples/RmlUi/Backends/RmlUi_Renderer_GL3.cpp:2044-2101`) -- a `GL_RGBA8`, `GL_LINEAR`-filtered, depth+stencil-backed texture framebuffer sized to exactly the same `(w, h)` as the viewport (`RMLUI_NUM_MSAA_SAMPLES=0` in this build, so no multisample-resolve scaling is involved) -- and clears THAT framebuffer, not FBO 0, before `render_standalone` ever calls the hook. Concretely: `RenderGl3::begin_frame` (`render_gl3.cpp:552-564`) clears FBO 0 FIRST, then hands control to `impl_->renderer.BeginFrame()`, which immediately rebinds the draw target away from FBO 0 to this internal layer -- so by the time the hook runs, `GL_DRAW_FRAMEBUFFER_BINDING` names RmlUi's own offscreen texture, not the window's default framebuffer.

**Why this is usually invisible, and when it is not.** RmlUi's own `EndFrame()` blits that internal layer 1:1 onto FBO 0 via a fullscreen-quad compositing pass (`examples/RmlUi/Backends/RmlUi_Renderer_GL3.cpp:903-917`) -- same dimensions, `GL_NEAREST`, no scaling -- so ordinary sprite/scene drawing composed inside the hook looks IDENTICAL on screen to drawing on FBO 0 directly, and `App::snapshot()`/the eventual `glfwSwapBuffers` both see the fully composited result. It matters ONLY for code inside the hook that inspects `GL_DRAW_FRAMEBUFFER_BINDING` and assumes it is `0`, or that calls `glReadPixels`/similar expecting to read back the real backbuffer mid-hook -- that code reads RmlUi's internal layer instead. `RenderGl3::end_frame` (`render_gl3.cpp:566-589`) confirms the OUTER contract stays true regardless: it calls `impl_->renderer.EndFrame()` (which itself rebinds FBO 0 for the compositing draw, and its own `EndFrame` restores every backed-up GL toggle it captured at `BeginFrame` -- `examples/RmlUi/Backends/RmlUi_Renderer_GL3.cpp:920-970`), then explicitly rebinds FBO 0 again for glintfx's own alpha-fix (forcing alpha to `1` so Wayland/Mutter never treats a translucent effect region as window transparency) -- so by the time `App::render()` returns, FBO 0 IS the real default framebuffer, holding the fully composited frame. The internal-layer binding is a fact about the WINDOW spanned between hook-entry and `EndFrame()`'s own compositing rebind, never observable from outside `App::render()`.

**PT:** A seção 21 já documenta que o hook do `set_frame_callback` é um lugar suportado para hospedar um renderer GL completo e independente. Esta seção responde às três perguntas que um consumidor migrando a própria casca dona-de-janela para o `glintfx::App` precisa ter respondidas pelo CÓDIGO, não descobertas por tentativa e erro: (a) o contexto GL está garantidamente corrente dentro do hook, (b) como um host resolve um ponteiro de função GL sem SDL/GLFW próprio, e (c) exatamente qual estado GL -- INCLUSIVE qual framebuffer -- está vinculado na entrada do hook e é garantido na saída. Nada abaixo sobrepõe a própria descrição de restauração de estado da seção 21; isto deixa explícita uma coisa que aquela descrição não soletra: a identidade do FRAMEBUFFER.

**(a) Sim, garantidamente corrente -- invariante de contexto único, citado.** `WindowGlfw::create()` chama `glfwMakeContextCurrent(win_)` exatamente uma vez (`window_glfw.cpp:185`), logo após `glfwCreateWindow`, e o próprio doc-comment de `WindowGlfw::set_swap_interval` declara o invariante do qual isto depende explicitamente: *"esta classe nunca o troca pelo resto da vida do App"* (`window_glfw.hpp:120-126`). Nenhuma outra chamada neste código-base retorna a mira do contexto pra uma janela/thread diferente pela vida de uma instância `App` (a seção 26 acima percorre a sequência de construção completa da qual este invariante faz parte). O hook de `App::set_frame_callback` (`app.hpp:568`) dispara de dentro de `Engine::render_standalone` (`engine.cpp:191-209`, chamado por `App::render()`), que sempre roda na MESMA thread em que o construtor do `App` rodou -- então o contexto que o `create()` tornou corrente é, incondicionalmente, ainda o corrente quando o hook roda.

**(b) `glintfx::gl_proc_address(const char* name)`** (`glintfx/include/glintfx/gl_proc.hpp`, só modo App, mesma guarda `GLINTFX_BACKEND_GLFW` do `app.hpp`) -- o equivalente ao `SDL_GL_GetProcAddress`. Uma delegação de uma linha (`glintfx/src/gl_proc.cpp`) pro `glx_gl_get_proc_address()`, um NOVO ponto de entrada de símbolo único no loader privado já existente (`glintfx/src/gl_loader.h`/`.c`) que reusa a MESMA cadeia de resolução `glX -> EGL -> dlsym` / `wglGetProcAddress -> GetProcAddress` que o próprio `glx_gl_load()` já usa por entrada de tabela -- zero lógica de resolução duplicada. Deliberadamente NÃO expõe o próprio `glx_gl_load()` (o ponto de entrada em lote) nem a tabela interna de ponteiros de função: os dois permanecem privados, então esta chamada é a ÚNICA costura que cruza a própria fronteira de prefixo `glx_` do loader (ADR-0013), resolvendo exatamente o símbolo que um chamador nomeia e nada além disso. `name` nulo/`""`, e uma chamada feita antes de qualquer `App`/`UiLayer` ter rodado `glx_gl_load()` ao menos uma vez no processo, os dois retornam `nullptr` -- fail-high documentado, nunca um crash. **Um nome inventado NÃO retorna `nullptr` de forma confiável, porém -- medido, não presumido:** sob o próprio ambiente de CI Mesa/llvmpipe desta biblioteca, `gl_proc_address("totally_bogus_symbol")` retornou um ponteiro NÃO-NULO. Este é o próprio comportamento documentado do `GLX_ARB_get_proc_address` (uma implementação PODE devolver um ponteiro não-nulo pra um nome que não reconhece; o chamador é esperado a já ter confirmado que a extensão/função existe antes de chamar o ponteiro devolvido), não um bug da glintfx -- a mesma ressalva que se aplica a chamar `glXGetProcAddressARB`/`SDL_GL_GetProcAddress` direto. Só passe um nome que você já sabe que deveria existir pro contexto GL 3.3 core-profile que a glintfx pede, ou uma extensão core/EXT/ARB confirmada presente separadamente (o próprio doc-comment de `glx_gl_get_proc_address`, `gl_loader.h`, tem o raciocínio completo por ramo).

**(c) Estado GL na entrada/saída do hook -- e a correção de identidade do framebuffer.** A própria nota "CONTRATO DE ESTADO GL" da seção 21/`engine.hpp` (`Engine::render_standalone`, `engine.cpp:191-209`) já diz que o hook pode deixar estado arbitrário para trás e é restaurado ao que "o próprio `BeginFrame()` do RmlUi tinha acabado de definir" via um snapshot/restore de `GlStateGuard` (`gl_state.hpp:63-158`) tirado logo antes do hook rodar e aplicado logo depois. O que essa base concretamente É, confirmado lendo o source pinado do RmlUi (`RenderInterface_GL3::BeginFrame`, `examples/RmlUi/Backends/RmlUi_Renderer_GL3.cpp:813-890`, commit `2cd28864ae25ed345b70598751703a5433b12356` conforme o `FetchContent_Declare(RmlUi ...)` do `glintfx/CMakeLists.txt`): viewport `(0, 0, w, h)`; `GL_SCISSOR_TEST`/`GL_CULL_FACE`/`GL_DEPTH_TEST` desabilitados; `GL_STENCIL_TEST` habilitado (`GL_ALWAYS`, ref `1`, mask todo-uns, ops `GL_KEEP`); `GL_BLEND` habilitado com a função de alpha premultiplicado (`glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA)`, equação `GL_FUNC_ADD`); unidade de textura ativa `GL_TEXTURE0`; nenhum programa de shader vinculado.

**O draw framebuffer vinculado na entrada do hook NÃO É o FBO 0** -- vale dizer em voz alta, porque o próprio comentário de topo-de-classe do `Engine` ("clear + UI + alpha-fix **no FBO0**", `engine.hpp:4`) poderia de outro modo ler como uma promessa que não é bem essa: `RenderInterface_GL3::BeginFrame()` empilha e vincula incondicionalmente uma render layer offscreen INTERNA bem antes de retornar (`RenderLayerStack::PushLayer`/`BeginFrame`, `examples/RmlUi/Backends/RmlUi_Renderer_GL3.cpp:2044-2101`) -- um framebuffer de textura `GL_RGBA8`, filtrado `GL_LINEAR`, com depth+stencil, dimensionado exatamente ao mesmo `(w, h)` do viewport (`RMLUI_NUM_MSAA_SAMPLES=0` neste build, então nenhum escalonamento de resolve multisample está envolvido) -- e limpa AQUELE framebuffer, não o FBO 0, antes do `render_standalone` sequer chamar o hook. Concretamente: `RenderGl3::begin_frame` (`render_gl3.cpp:552-564`) limpa o FBO 0 PRIMEIRO, depois entrega o controle a `impl_->renderer.BeginFrame()`, que revincula imediatamente o alvo de desenho, tirando-o do FBO 0 pra esta layer interna -- então, quando o hook roda, `GL_DRAW_FRAMEBUFFER_BINDING` nomeia a própria textura offscreen do RmlUi, não o framebuffer default da janela.

**Por que isto costuma ser invisível, e quando não é.** O próprio `EndFrame()` do RmlUi faz o blit dessa layer interna 1:1 pro FBO 0 via um passe de composição em quad fullscreen (`examples/RmlUi/Backends/RmlUi_Renderer_GL3.cpp:903-917`) -- mesmas dimensões, `GL_NEAREST`, sem escalonamento -- então um desenho comum de sprite/cena composto dentro do hook parece IDÊNTICO na tela a desenhar direto no FBO 0, e o `App::snapshot()`/o eventual `glfwSwapBuffers` veem os dois o resultado composto completo. Isto importa SÓ pra código dentro do hook que inspeciona `GL_DRAW_FRAMEBUFFER_BINDING` e assume que é `0`, ou que chama `glReadPixels`/similar esperando ler de volta o backbuffer real no meio do hook -- esse código lê a layer interna do RmlUi em vez disso. `RenderGl3::end_frame` (`render_gl3.cpp:566-589`) confirma que o contrato EXTERNO permanece verdadeiro de qualquer forma: chama `impl_->renderer.EndFrame()` (que por si só revincula o FBO 0 pro desenho de composição, e o próprio `EndFrame` dele restaura todo toggle GL que fez backup no `BeginFrame` -- `examples/RmlUi/Backends/RmlUi_Renderer_GL3.cpp:920-970`), depois revincula o FBO 0 explicitamente de novo pro próprio alpha-fix da glintfx (forçando alpha pra `1` para que o Wayland/Mutter nunca trate uma região de efeito translúcida como transparência de janela) -- então, quando `App::render()` retorna, o FBO 0 É o framebuffer default real, guardando o frame composto completo. O binding da layer interna é um fato sobre a JANELA aberta entre a entrada do hook e a revinculação de composição do próprio `EndFrame()`, nunca observável de fora do `App::render()`.

## 28. Composited-frame readback in embed mode (`UiLayer::capture_frame()`, `FRAMEGRAB-EMBED`) / Readback do frame composto em modo embed (`UiLayer::capture_frame()`, `FRAMEGRAB-EMBED`)

**EN:** `App::capture_frame()` shipped `FRAMEGRAB-TEX` for `glintfx::App` only (section header of `app.hpp`'s own `CapturedFrame` doc-comment) -- the embed-only build (`GLINTFX_BACKEND_GLFW=OFF`) had zero access to it, and the consumer who asked for the capability (GusWorld) is embed-only. `UiLayer::capture_frame()` (`ui_layer.hpp:900`) closes that gap: same struct shape, same pixel-format/ownership/fail-high contract as `App::CapturedFrame` (`ui_layer.hpp:907`), reading the SAME shared readback both facades wrap, `Engine::capture_frame` (`engine.hpp:202`) -- implemented at `engine.cpp:573`. `App::capture_frame()` (`app.cpp:497`) was itself refactored to delegate to it instead of duplicating the `glReadPixels`/flip/RGBA8-expansion body inline, so the two facades cannot drift.

**Region captured -- the declared ceiling.** `capture_frame()` reads the CURRENT viewport region set by `set_viewport()` (section 10 above), not the whole render target. In the common case (`set_viewport(w, h)`, the 2-arg overload) that IS the whole window. In letterbox mode (`set_viewport(x, y, w, h, target_h)`), only the sub-region is captured -- whatever the host drew OUTSIDE it (its own letterbox bars) is not in the returned buffer; a host that wants the full render target reads it itself.

**Timing -- the one rule that matters.** Call `capture_frame()` AFTER `render()` and BEFORE the host's own buffer swap. Unlike `App::capture_frame()` (which drives its own render pass AND its own swap), `UiLayer::capture_frame()` renders nothing and swaps nothing -- it is a pure read of whatever FBO 0 already holds, mirroring the "host owns clear and swap" discipline of section 5 above. Calling it before `render()` reads the host's own pre-UI drawing (legitimate, just not "the composited frame"); calling it after the host's own swap reads undefined back-buffer content on many GL implementations, Mesa/llvmpipe included -- the same reasoning `App::snapshot()`'s own doc-comment already states.

**No loaded document required.** `capture_frame()` reads FBO 0 regardless of whether `load()` was ever called -- it captures whatever the host itself drew, with or without glintfx UI content composited on top.

**GL state.** Automatically saved and restored -- but NOT via the `GlStateGuard` `render()`'s own compose-only contract uses (section 4 above): a plain pixel readback touches a DIFFERENT 5 pieces of GL state (`GL_READ_FRAMEBUFFER` binding, `GL_READ_BUFFER`, `GL_PACK_ALIGNMENT`, `GL_PACK_ROW_LENGTH`, the `GL_PIXEL_PACK_BUFFER` binding), none of which the render-pass guard captures. `GL_PACK_ALIGNMENT` is forced to `1` for the read itself -- the fix for a real, measured heap-corruption bug class: at the GL default alignment of `4`, a read whose row byte count (`width * 3` for RGB) is NOT a multiple of `4` pads each row to the next 4-byte boundary, which `glReadPixels` accounts for internally but a tightly-packed destination buffer sized `width*height*3` does not have room for -- **the class is "width not a multiple of 4", not "odd width"** (review adversarial's own reclassification, `w21_framegrab_embed` thread, 2026-07-30: `gcd(3,4)=1`, so an EVEN width that is not a multiple of `4`, e.g. `2`/`6`/`198`, corrupts the exact same way; the forced `GL_PACK_ALIGNMENT=1` fix is unconditional and already covers the whole class -- only the earlier framing/test name were too narrow). Confirmed TWICE, with two different failure signatures for the same underlying corruption: deliberately reverting the forced alignment during this fatia's own review and rebuilding crashed the width-not-multiple-of-4 test (`tests/ui_layer_capture_frame_smoke.cpp`, Part F) with `double free or corruption (!prev)`; the review adversarial's own sweep of `W ∈ {1,2,3,4,5,198,301}` under the same sabotage got a bare `SIGSEGV` instead -- same heap corruption, landing on a different allocator metadata byte depending on allocation history/width. Neither crash is "the" reproduction; both are the same bug.

**Fail-high.** `!ok()`, or a degenerate current viewport (`w<=0||h<=0`), returns `CapturedFrame{}` (`ok=false`) without touching GL. `set_viewport()`'s own guard rejects `w<=0||h<=0` in normal use (keeps the previous viewport), so this branch is NOT reachable that way through it. **CORRECTED 2026-07-30 (`UILAYER-CTOR-GUARD`, `4244b3d`) -- this paragraph used to describe the OPPOSITE of current behaviour and said so under a "measured, not assumed" claim that itself went stale:** `UiLayerConfig` is now validated by the constructor BEFORE `Engine::attach()` is ever called (same ceiling/floor `set_viewport()` enforces) -- `UiLayerConfig{.logical_width = 0, ...}` now leaves `ok() == false` immediately, an `LT_ERROR` logged, and `Rml::CreateContext` is never reached; it does NOT reach RmlUi unchecked and does NOT leave `ok() == true` the way this paragraph used to claim. Coverage moved with the fix: `tests/ui_layer_capture_frame_smoke.cpp`'s Part G (that this paragraph used to cite as exercising "exactly this path") was split into G1 (proves the new ctor guard: `ok() == false`, `capture_frame()` a safe no-op on it) and G2 (proves `Engine::capture_frame`'s own `w<=0||h<=0` guard directly, bypassing `UiLayer` entirely, since the path through it that G1's predecessor used no longer exists) -- see `ui_layer_capture_frame_smoke.cpp`'s own doc-comment for the full re-derivation. **The lesson this correction is itself an example of:** a paragraph claiming to have MEASURED a behaviour is the one a reader is least likely to re-verify -- it is the phrase that tells a reader to stop checking, because someone already checked; when the measurement ages past the code that produced it and the text does not say so, that is a stronger false claim than an unfulfilled promise, because a promise a skeptical reader might doubt, a stated measurement they will not.

**Edge case explicitly NOT covered, and why.** A `capture_frame()` call on a moved-from `UiLayer` (`UiLayer b = std::move(a); a.capture_frame();`) segfaults -- confirmed by gdb during this fatia's own review. This is NOT specific to `capture_frame()`: `ui_layer.hpp`'s own class-level doc-comment states that calling any method other than `ok()`/the destructor on a moved-from `UiLayer` is undefined behaviour, "same convention as `std::unique_ptr`", and every pre-existing method (`get_element_box`, `scroll_element_into_view`, ~30 of them) shares the exact same `if (!impl_->ok)` guard shape with no `impl_ &&` check -- `capture_frame()` simply matches the established convention rather than inventing a new, inconsistent one. Flagged as a possible class-wide hardening follow-up (add `impl_ &&` to every guard, not just this new method), not fixed unilaterally in this fatia.

**What this call does NOT depend on (`DOC-NAO-DEPENDE`, W22 S6).** The "Timing" paragraph above documents ONE use case -- capturing the frame WITH glintfx UI content composited on top -- worded as "call after `render()`". A consumer read that as a REQUIREMENT, concluded the capability did not serve a different case they had (grabbing the host's own scene with NO UI on it, to build a paused/dialog-background texture), and nearly requested a new API for "the host's default composited framebuffer" -- which is exactly what `UiLayer::capture_frame()` already returns when called BEFORE `render()`. As the consumer put it, and it is worth repeating verbatim: *"the absence of a statement is not the opposite statement"* -- nobody wrote that this call depends on `render()` having run, because it does not, but nobody wrote that it does NOT depend on it either, and silence read as a limitation where there was freedom. Stated plainly now: `capture_frame()` does not require `render()` to have been called, ever, on that `UiLayer` -- it never touches RmlUi, it is pure readback of whatever FBO 0 already holds in the current viewport region. Calling it BEFORE `render()` is a first-class, supported use case (the host's own scene, UI-free), not a misuse of a call meant for after.

**Measured, not assumed (consumer-verified 2026-07-30, `DOC-NAO-DEPENDE`).** Two facts the consumer's own pixel-level verification established, both worth documenting because they change what "before `render()`" means in practice: **(1)** a capture taken BETWEEN `update()` and `render()` is BYTE-IDENTICAL to one taken before both were called -- `render()` is the ONLY point in the frame lifecycle that composites anything into FBO 0, so "before `render()`" is the useful guarantee to document, not "before the `update()`/`render()` pair". **(2)** the verification method itself matters as much as the result: 12 pixel criteria, all PASS, read from the RAW BUFFERS ON DISK (not from the producing process's own memory), with the checker enumerating the FULL histogram of each region rather than searching for an expected colour -- and it ran two NEGATIVE CONTROLS by mutating the captured data with the two plausible failure modes (`before := after`, "returned ok but with the UI still inside"; `before := zeros`, "returned ok but empty") and confirmed BOTH are correctly rejected. One contra-intuitive detail from that exercise, worth carrying forward as a testing lesson: the `before := after` mutant PASSES the naive check "pixels differ outside the panel region" (they trivially do not differ anywhere) -- "the two images differ" does not, by itself, answer the question; what actually catches that mutant is requiring the host's OWN scene content to be 100% present exactly where the UI would otherwise be.

**⚠️ THE BACK-BUFFER RESIDUE, and it is what bites whoever uses this capability for what it unlocks.** On Mesa/llvmpipe (and this is a property of the GL implementation, not glintfx's own code) the buffer swap behaves like a COPY, not a flip -- the back buffer RETAINS whatever frame was composited into it last, across swaps. If a host calls `capture_frame()` before `render()` on a frame where it does NOT repaint (clear + redraw) the captured region -- easy to do exactly in a transition frame, which is the most likely moment anyone reaches for a "no-UI" capture in the first place -- the call returns the PREVIOUS frame's content, UI included, not a fresh UI-free read. This LOOKS like a bug in `capture_frame()` (or in this very guarantee) when it is actually the host not having painted that frame yet. This is distinct from, and easier to hit than, the "read after the host's own swap is undefined" case the Timing paragraph above already covers -- that one is about reading AFTER the wrong point; this one is about reading a stale composite from a point the host simply has not drawn into yet this frame. Whoever captures before `render()` must ensure the host has ALREADY cleared and drawn its own scene into FBO 0 in that SAME frame, or the read is stale, not UI-free.

**PT:** O `App::capture_frame()` lançou o `FRAMEGRAB-TEX` só pra `glintfx::App` (cabeçalho de seção do próprio doc-comment de `CapturedFrame` em `app.hpp`) -- o build embed-only (`GLINTFX_BACKEND_GLFW=OFF`) tinha zero acesso a ele, e o consumidor que pediu a capacidade (GusWorld) é embed-only. O `UiLayer::capture_frame()` (`ui_layer.hpp:900`) fecha essa lacuna: mesma forma de struct, mesmo contrato de formato-de-pixel/posse/fail-high que `App::CapturedFrame` (`ui_layer.hpp:907`), lendo o MESMO readback compartilhado que as duas fachadas encapsulam, `Engine::capture_frame` (`engine.hpp:202`) -- implementado em `engine.cpp:573`. O `App::capture_frame()` (`app.cpp:497`) foi ele mesmo refatorado pra delegar a ele em vez de duplicar o corpo `glReadPixels`/flip/expansão-RGBA8 inline, então as duas fachadas não conseguem derivar.

**Região capturada -- o teto declarado.** `capture_frame()` lê a região de viewport CORRENTE definida por `set_viewport()` (seção 10 acima), não o render target inteiro. No caso comum (`set_viewport(w, h)`, a sobrecarga de 2 args) isso É a janela inteira. Em modo letterbox (`set_viewport(x, y, w, h, target_h)`), só a sub-região é capturada -- o que quer que o host tenha desenhado FORA dela (as próprias barras de letterbox) não está no buffer devolvido; um host que quer o render target inteiro lê ele mesmo.

**Timing -- a única regra que importa.** Chame `capture_frame()` DEPOIS de `render()` e ANTES do próprio swap de buffer do host. Diferente do `App::capture_frame()` (que conduz o próprio passe de render E o próprio swap), o `UiLayer::capture_frame()` não renderiza nada e não faz swap nenhum -- é uma leitura pura do que o FBO 0 já guarda, espelhando a disciplina "host é dono do clear e do swap" da seção 5 acima. Chamá-lo antes do `render()` lê o próprio desenho pré-UI do host (legítimo, só não "o frame composto"); chamá-lo depois do próprio swap do host lê conteúdo de back-buffer indefinido em muitas implementações GL, Mesa/llvmpipe inclusive -- o mesmo racional que o próprio doc-comment de `App::snapshot()` já declara.

**Nenhum documento carregado exigido.** `capture_frame()` lê o FBO 0 independente de `load()` ter sido chamado alguma vez -- captura o que quer que o próprio host tenha desenhado, com ou sem conteúdo de UI da glintfx composto por cima.

**Estado GL.** Salvo e restaurado automaticamente -- mas NÃO via o `GlStateGuard` que o próprio contrato compose-only do `render()` usa (seção 4 acima): um readback de pixel puro toca 5 peças de estado GL DIFERENTES (binding de `GL_READ_FRAMEBUFFER`, `GL_READ_BUFFER`, `GL_PACK_ALIGNMENT`, `GL_PACK_ROW_LENGTH`, binding de `GL_PIXEL_PACK_BUFFER`), nenhuma das quais o guard do passe de render captura. `GL_PACK_ALIGNMENT` é forçado a `1` pra própria leitura -- o conserto pra uma classe de bug de corrupção de heap real e medida: no alinhamento default do GL de `4`, uma leitura cuja contagem de bytes por linha (`width * 3` pra RGB) NÃO é múltipla de `4` preenche cada linha até o próximo limite de 4 bytes, o que o próprio `glReadPixels` leva em conta internamente mas um buffer de destino compactado dimensionado `width*height*3` não tem espaço para -- **a classe é "largura não múltipla de 4", não "largura ímpar"** (reclassificação do próprio review adversarial, thread do `w21_framegrab_embed`, 2026-07-30: `mdc(3,4)=1`, então uma largura PAR que não é múltipla de `4`, ex. `2`/`6`/`198`, corrompe exatamente do mesmo jeito; o conserto forçado `GL_PACK_ALIGNMENT=1` é incondicional e já cobre a classe inteira -- só o enquadramento/nome de teste anteriores eram estreitos demais). Confirmado DUAS VEZES, com duas assinaturas de falha diferentes pra mesma corrupção subjacente: reverter deliberadamente o alinhamento forçado durante o próprio review desta fatia e rebuildar crashou o teste de largura-não-múltipla-de-4 (`tests/ui_layer_capture_frame_smoke.cpp`, Parte F) com `double free or corruption (!prev)`; a própria varredura do review adversarial de `W ∈ {1,2,3,4,5,198,301}` sob a mesma sabotagem obteve um `SIGSEGV` puro em vez disso -- a mesma corrupção de heap, caindo num byte diferente de metadado do alocador dependendo do histórico de alocação/largura. Nenhum dos dois crashes é "a" reprodução; os dois são o mesmo bug.

**Fail-high.** `!ok()`, ou um viewport corrente degenerado (`w<=0||h<=0`), retorna `CapturedFrame{}` (`ok=false`) sem tocar GL. O próprio guard de `set_viewport()` rejeita `w<=0||h<=0` em uso normal (mantém o viewport anterior), então este ramo NÃO é alcançável por esse caminho através dele. **CORRIGIDO 2026-07-30 (`UILAYER-CTOR-GUARD`, `4244b3d`) -- este parágrafo descrevia o OPOSTO do comportamento atual, e dizia isso sob uma alegação "medido, não presumido" que ela mesma envelheceu:** o `UiLayerConfig` agora é validado pelo construtor ANTES de `Engine::attach()` ser sequer chamado (mesmo teto/piso que `set_viewport()` enforça) -- `UiLayerConfig{.logical_width = 0, ...}` agora deixa `ok() == false` imediatamente, um `LT_ERROR` é logado, e `Rml::CreateContext` nunca é alcançado; NÃO chega ao RmlUi sem checagem e NÃO deixa `ok() == true` do jeito que este parágrafo costumava afirmar. A cobertura se moveu junto com o conserto: a Parte G de `tests/ui_layer_capture_frame_smoke.cpp` (que este parágrafo costumava citar como quem "exercita exatamente este caminho") foi dividida em G1 (prova o guard novo do ctor: `ok() == false`, `capture_frame()` um no-op seguro nele) e G2 (prova o próprio guard `w<=0||h<=0` de `Engine::capture_frame` diretamente, contornando o `UiLayer` por completo, já que o caminho por ele que a antecessora do G1 usava não existe mais) -- ver o próprio doc-comment de `ui_layer_capture_frame_smoke.cpp` pra re-derivação completa. **A lição da qual esta correção é ela mesma um exemplo:** um parágrafo alegando ter MEDIDO um comportamento é o que um leitor tem menos chance de reverificar -- é a frase que diz ao leitor pra parar de checar, porque alguém já checou; quando a medição envelhece além do código que a produziu e o texto não avisa, isso é uma afirmação falsa mais forte que uma promessa não cumprida, porque uma promessa um leitor cético pode duvidar, uma medição declarada ele não vai.

**Caso-limite explicitamente NÃO coberto, e por quê.** Uma chamada a `capture_frame()` num `UiLayer` movido-de (`UiLayer b = std::move(a); a.capture_frame();`) dá segfault -- confirmado por gdb durante o próprio review desta fatia. Isto NÃO é específico do `capture_frame()`: o próprio comentário de nível de classe de `ui_layer.hpp` declara que chamar qualquer método além de `ok()`/o destrutor num `UiLayer` movido-de é comportamento indefinido, "mesma convenção do `std::unique_ptr`", e todo método pré-existente (`get_element_box`, `scroll_element_into_view`, ~30 deles) compartilha exatamente o mesmo formato de guard `if (!impl_->ok)` sem checagem `impl_ &&` -- o `capture_frame()` só bate com a convenção estabelecida em vez de inventar uma nova, inconsistente. Sinalizado como possível desdobramento de hardening em nível de classe (somar `impl_ &&` em todo guard, não só neste método novo), não consertado unilateralmente nesta fatia.

**O que esta chamada NÃO depende (`DOC-NAO-DEPENDE`, W22 S6).** O parágrafo "Timing" acima documenta UM caso de uso -- capturar o frame COM conteúdo de UI da glintfx composto por cima -- redigido como "chame depois do `render()`". Um consumidor leu aquilo como REQUISITO, concluiu que a capacidade não servia a um caso diferente que ele tinha (capturar a própria cena do host SEM UI nenhuma, pra construir uma textura de fundo de pausa/diálogo), e ia pedir uma API nova pra "o framebuffer composto default do host" -- que é EXATAMENTE o que `UiLayer::capture_frame()` já devolve quando chamado ANTES do `render()`. Nas palavras dele, que vale repetir ao pé da letra: *"a ausência de uma afirmação não é a afirmação contrária"* -- ninguém escreveu que esta chamada depende do `render()` ter rodado, porque não depende, mas também ninguém escreveu que ela NÃO depende, e o silêncio foi lido como limitação onde havia liberdade. Dito com todas as letras agora: `capture_frame()` não exige que `render()` tenha sido chamado, nunca, naquele `UiLayer` -- não toca o RmlUi em nenhum momento, é readback puro do que o FBO 0 já guarda na região de viewport corrente. Chamá-lo ANTES do `render()` é um caso de uso de primeira classe, suportado (a própria cena do host, sem UI), não um uso indevido de uma chamada pensada só pra depois.

**Medido, não presumido (verificado pelo consumidor 2026-07-30, `DOC-NAO-DEPENDE`).** Dois fatos que a própria verificação em nível de pixel do consumidor estabeleceu, os dois valendo documentar porque mudam o que "antes do `render()`" significa na prática: **(1)** uma captura feita ENTRE `update()` e `render()` é BYTE-IDÊNTICA a uma feita antes dos dois -- `render()` é o ÚNICO ponto do ciclo de vida do frame que compõe qualquer coisa no FBO 0, então "antes do `render()`" é a garantia útil a documentar, não "antes do par `update()`/`render()`". **(2)** o próprio método de verificação importa tanto quanto o resultado: 12 critérios de pixel, todos PASS, lidos dos BUFFERS CRUS EM DISCO (não da memória do próprio processo que os produziu), com o verificador enumerando o HISTOGRAMA COMPLETO de cada região em vez de procurar a cor esperada -- e rodou DOIS CONTROLES NEGATIVOS mutando os dados capturados com os dois modos de falha plausíveis (`before := after`, "retornou ok mas com a UI ainda dentro"; `before := zeros`, "retornou ok mas vazio") e confirmou que os DOIS são rejeitados corretamente. Um detalhe contraintuitivo desse exercício, vale carregar como lição de teste: o mutante `before := after` PASSA na checagem ingênua "pixels diferem fora da região do painel" (trivialmente não diferem em lugar nenhum) -- "as duas imagens diferem" não responde a pergunta sozinho; o que de fato mata esse mutante é exigir que o conteúdo da PRÓPRIA cena do host esteja 100% presente exatamente onde a UI estaria.

**⚠️ O RESÍDUO DE BACK BUFFER, e é o que morde quem usa esta capacidade para o que ela destrava.** Em Mesa/llvmpipe (e isto é propriedade da implementação GL, não código próprio da glintfx) o swap de buffer se comporta como uma CÓPIA, não um flip -- o back buffer RETÉM o que quer que tenha sido composto nele por último, através dos swaps. Se um host chama `capture_frame()` antes do `render()` num quadro em que NÃO repinta (limpa + redesenha) a região capturada -- fácil de acontecer justo num quadro de transição, que é o momento mais provável de alguém buscar uma captura "sem UI" em primeiro lugar -- a chamada devolve o conteúdo do quadro ANTERIOR, UI incluída, não uma leitura fresca sem UI. Isto PARECE um bug no `capture_frame()` (ou nesta própria garantia) quando na verdade é o host ainda não ter pintado aquele quadro. É distinto, e mais fácil de cair nele, do que o caso "ler depois do próprio swap do host é indefinido" que o parágrafo Timing acima já cobre -- aquele é sobre ler DEPOIS do ponto errado; este é sobre ler um composto obsoleto de um ponto no qual o host simplesmente ainda não desenhou naquele quadro. Quem captura antes do `render()` precisa garantir que o host JÁ limpou e desenhou a própria cena no FBO 0 NAQUELE MESMO quadro, ou a leitura é obsoleta, não sem-UI.

## 29. Two `UiLayer`/`App` instances in one process corrupt global state (`UILAYER-SINGLETON`) / Dois `UiLayer`/`App` no mesmo processo corrompem estado global (`UILAYER-SINGLETON`)

**EN:** ⚠️ Constructing a SECOND `UiLayer` (or `App`) while a FIRST one is still alive, in the SAME process, is **not supported and corrupts global state -- silently**, not merely "untested". This section states that fact and its exact failure shape; a real guard against it is not built yet (see "Scope" below). Full verification chain against the pinned RmlUi source, and the exact class-level comment stating the same, live at `ui_layer.hpp` (end of the class, `UILAYER-SINGLETON` block).

**Why nothing catches it.** Every `RMLUI_ASSERTMSG` in the vendored RmlUi is a NO-OP in our builds (`RMLUI_DEBUG` is never defined -- confirmed across the RmlUi CMake files, this project's own, and the actually-compiled `flags.make`; `CMAKE_BUILD_TYPE` is empty in the cache). Wherever upstream RmlUi believed it had a defensive check against a misuse like this, this library has none at all: the code runs straight through over already-initialised global state instead of asserting. **The lesson generalises beyond this one item: anywhere this codebase is counting on an `RMLUI_ASSERT*` to catch invalid use, it is counting on nothing.** Worth its own sweep some day; not attempted here.

**The mechanism, verified line-by-line against the pinned RmlUi source (`Core.cpp`, `ControlledLifetimeResource.h`).** Each `UiLayer`/`App` owns its own `Engine`, which owns its own `Bootstrap` BY VALUE (`engine.cpp`) -- there is no shared `Bootstrap` and no refcount anywhere in THIS library. But `Rml::Initialise()`/`Rml::Shutdown()` and the `Rml::Context` name are RmlUi's own process-wide globals. `Rml::Initialise()` calls `core_data.Initialize()` -- `Initialize()`, NOT the sibling `InitializeIfEmpty()` sitting right next to it in the same header, a choice that is meaningful -- which does `pointer = new T();` UNCONDITIONALLY, guarded only by the no-op assert above. A second `Initialise()` therefore REPLACES the global `core_data` WITHOUT deleting the first: the first `CoreData` (its `contexts` map with the "main" `Context` inside it, `render_managers`, the `default_*_interface` pointers) leaks -- still alive in memory, unreachable from RmlUi's own global state from that point on. Consequence: the duplicate-name check inside `CreateContext` reads `core_data->contexts`, which has just become a NEW, EMPTY map -- so the second instance's `CreateContext("main", ...)` (hardcoded name, `bootstrap.cpp`, no per-instance parametrisation) SUCCEEDS. It does not fail on a name collision, which is the natural but wrong hypothesis for what would break first. The first instance is notified of nothing and ends up cross-contaminated ASYMMETRICALLY: `SystemInterface` and `FontEngineInterface` are read LIVE on every use (process-wide globals, now overwritten -- the first instance's animation timing and any subsequent `LoadFontFace` call are served by the SECOND instance's engine from that moment on), but each `Context`'s own `RenderManager` was captured BY REFERENCE at the moment that Context was created, so the first instance's OWN render output keeps working correctly. This is not "renders through the wrong backend" -- it is a MIX, different subsystems drifting to different owners independently of each other.

**⚠️ The teardown asymmetry, stated with the exact words it needs to be remembered by, because the natural reading of the warning above is backwards: the first instance's destructor brings down the second.** Whichever `UiLayer`/`App` calls `Rml::Shutdown()` first destroys whichever `core_data` is CURRENT at that moment -- and after the replacement described above, "current" belongs to the LAST instance initialised, not to the one calling Shutdown. A reader who takes in the paragraph above and concludes "then I will destroy them in the right order" is reaching for exactly the wrong lever: there is no destruction order that makes two live instances safe, because the danger is in CONSTRUCTING a second one at all, not in the order two already-corrupted instances get torn down. Compounding this: `Bootstrap::init()` already calls `Rml::Shutdown()` GLOBALLY AND UNCONDITIONALLY (`bootstrap.cpp`) when ONLY that one instance's own `CreateContext` call fails -- turning a LOCAL failure (this one instance could not get a context) into a PROCESS-WIDE one (every OTHER live instance loses its own RmlUi state too), because that transforms a local error into a process failure, and that is exactly the kind of thing that surfaces in production, not in a unit test.

**Scope decided for this onda (W22), and why.** This onda delivers ONLY this warning -- declaring the hazard, its silent failure mode, and the teardown asymmetry. The REAL guard (a static counter in `Bootstrap::init()`/`shutdown()` refusing a second `init()` while the first is still alive, and making the unconditional `Rml::Shutdown()` above conditional on being the LAST live instance) is `UILAYER-SINGLETON-GUARD`, a W23 item -- a behaviour change earning its own fatia with its own mutation proof, per this onda's own rule that scope growing mid-onda becomes next onda's item, not this one's. The reasoning for shipping the warning alone, now: a doc saying "not supported" is TRUE and ships zero risk; code promising a guard it does not have would repeat, inside this very item, the exact defect class this onda spent a night hunting down elsewhere in the codebase (a promise in prose the code does not keep).

**The consumer's own case, verified in their repo, and it is safer than they first reported.** The most likely real-world coexistence case -- a pause menu opening a nested save/load screen, each backed by its own `UiLayer` -- is safe by STRUCTURAL design, not by ordering discipline. `screen_state.hpp` (GusWorld) documents that `run_screen_state(parent)` must RETURN (running `parent.exit()`, which releases the parent's `UiLayer`) before any code can call `run_screen_state(child)` -- the guarantee is C++'s own function-call sequencing, not something the child screen's author has to remember. The consumer reported this as "`reset()` runs first, plus a comment explains it" -- the actual guarantee is stronger than that framing suggests. A measured note passed back to them, not a verdict: they described "six screen classes"; a count of `screens/` files found **10** referencing `UiLayer` and **7** going through `run_screen_state` -- the difference (`system_menu_rml`, `save_load_menu_rml`, `battle_cockpit_rml`, `battle_input`) is where their own sweep of the wider surface should start.

**🔴 THIS HAS A PRODUCTION VICTIM, not a theoretical risk (found by GusWorld, 2026-07-30).** Opening a second `UiLayer` while a first was still alive crashed FOR REAL, live: `screen_state.hpp` in the consumer's own repo documents that this happened, with the RmlUi log `"Element meta pool not empty on shutdown, 75 object(s) leaked"`, traced to a revert commit in their history (a pause menu opened inside a battle screen, reverted the same day it landed, 2026-07-05). The string was confirmed in the pinned RmlUi we actually link (`ElementMeta.cpp`), and the full message text is: *"Element meta pool not empty on shutdown, %d object(s) leaked. This will likely lead to a crash when element is destroyed. Ensure that no Rml::Element objects are kept alive in user space at the end of Rml::Shutdown."*

**⚠️ THE FINDING THAT MATTERS MORE THAN THE SYMPTOM: THE UPSTREAM MESSAGE MISDIRECTS.** It tells the consumer to *"ensure that no `Rml::Element` objects are kept alive in user space"* -- it blames the user for holding onto `Element`s. In the real mechanism described above, nobody held onto anything: the second `Initialise()` swapped `core_data` out, and those 75 objects became orphans of a `Context` the global RmlUi state no longer knows about. **Whoever follows the message's own advice will audit their own code for a leak that is not there, and find nothing.** A vague symptom makes someone search everywhere; a symptom pointing in the WRONG direction makes someone search hard in the wrong place -- and their own competence then works against them: the more rigorous the audit of their own code, the more confident they become the bug is not theirs, which delays rather than speeds up doubting the real premise. Displaced two ways at once: in TIME (the message fires at teardown, far from the constructor call that actually caused it, after everything appeared to work) -- *"the message is not the defect, it is its echo at teardown"* -- and in SPACE, pointing at entirely the wrong code.

**⚠️ IF YOU SEE THIS MESSAGE:** its own instruction will tell you to go hunt for live `Rml::Element`s in your own code. **DO NOT do that first** -- confirm first that there is no second `UiLayer`/`App` alive in the same process. Only audit your own code for a retained `Element` after ruling that out.

**⚠️ MANDATORY PHRASING for this class of warning (the consumer's own proposal, accepted):** do not write "the symptom is X, the cause is Y" -- write **"the symptom is X, it will tell you to do Z, DO NOT, confirm Y first"** (exactly the instruction in the paragraph above) -- because whoever reaches this point already has the misdirecting message on screen, and the first thing they need is **permission to not follow its advice**, not just the correct explanation somewhere further down. One more consequence of the same no-op-assert finding in this section: immediately after that log line, `ElementMeta.cpp` has an `RMLUI_ERROR` upstream clearly meant to abort on there too -- also a no-op in our builds, so the crash upstream expected to be loud is silent here as well.

**A note on citing third parties.** A corroborating reference the consumer initially gave (a `file:line` in their own repo) was already DEAD by the time it was checked -- the file had moved on since the original comment was written, and the cited lines today have no relation to the finding. The claim still holds, verified independently by other means, but this doc cites the SYMPTOM and the MECHANISM, never a third-party repo's `file:line` that moves -- exactly the same class of citation drift that blocked this whole onda's morning, just in a repo this library does not control.
...
**O mecanismo, verificado linha a linha contra o source pinado do RmlUi (`Core.cpp`, `ControlledLifetimeResource.h`).** Cada `UiLayer`/`App` possui o próprio `Engine`, que possui o próprio `Bootstrap` POR VALOR (`engine.cpp`) -- não há `Bootstrap` compartilhado nem refcount em lugar nenhum DESTA biblioteca. Mas `Rml::Initialise()`/`Rml::Shutdown()` e o nome do `Rml::Context` são globais de processo do PRÓPRIO RmlUi. `Rml::Initialise()` chama `core_data.Initialize()` -- `Initialize()`, NÃO o irmão `InitializeIfEmpty()` sentado bem ao lado no mesmo header, uma escolha significativa -- que faz `pointer = new T();` INCONDICIONAL, guardado só pelo assert no-op acima. Um segundo `Initialise()` portanto SUBSTITUI o `core_data` global SEM deletar o primeiro: o primeiro `CoreData` (o próprio mapa `contexts` com o `Context` "main" dentro dele, `render_managers`, os ponteiros `default_*_interface`) vaza -- continua vivo na memória, inalcançável pelo próprio estado global do RmlUi a partir daquele ponto. Consequência: a checagem anti-duplicata dentro de `CreateContext` lê `core_data->contexts`, que acabou de virar um mapa NOVO e VAZIO -- então o `CreateContext("main", ...)` da segunda instância (nome hardcoded, `bootstrap.cpp`, sem parametrização por instância) TEM SUCESSO. Não falha por colisão de nome, que é a hipótese natural mas errada pro que quebraria primeiro. A primeira instância não é avisada de nada e fica cross-contaminada de forma ASSIMÉTRICA: `SystemInterface` e `FontEngineInterface` são lidos AO VIVO a cada uso (globais de processo, agora sobrescritos -- o timing de animação da primeira instância e qualquer chamada `LoadFontFace` seguinte passam a ser servidos pelo motor da SEGUNDA instância a partir daquele momento), mas o próprio `RenderManager` de cada `Context` foi capturado POR REFERÊNCIA no momento em que aquele Context foi criado, então o PRÓPRIO render output da primeira instância continua funcionando corretamente. Isto não é "renderiza pelo backend errado" -- é uma MISTURA, subsistemas diferentes derivando para donos diferentes de forma independente um do outro.

**⚠️ A assimetria do teardown, dita com as palavras exatas com que precisa ser lembrada, porque a leitura natural do aviso acima é ao contrário: o destrutor do primeiro derruba o segundo.** Quem quer que chame `Rml::Shutdown()` primeiro destrói o `core_data` que estiver CORRENTE naquele momento -- e depois da substituição descrita acima, "corrente" pertence à ÚLTIMA instância inicializada, não à que está chamando Shutdown. Um leitor que absorve o parágrafo acima e conclui "então eu destruo na ordem certa" está puxando exatamente a alavanca errada: não há ordem de destruição que torne duas instâncias vivas seguras, porque o perigo está em CONSTRUIR uma segunda, não na ordem em que duas instâncias já corrompidas são desfeitas. Agravando isto: `Bootstrap::init()` já chama `Rml::Shutdown()` GLOBAL e INCONDICIONAL (`bootstrap.cpp`) quando SÓ a própria chamada `CreateContext` daquela instância falha -- transformando uma falha LOCAL (esta instância não conseguiu um contexto) numa falha de PROCESSO INTEIRO (toda OUTRA instância viva perde o próprio estado do RmlUi também), porque isso transforma um erro local em falha do processo, e é exatamente o tipo de coisa que aparece em produção, não num teste unitário.

**Escopo decidido para esta onda (W22), e por quê.** Esta onda entrega SÓ este aviso -- declarando o perigo, o próprio modo de falha silencioso, e a assimetria do teardown. O guard DE VERDADE (um contador estático em `Bootstrap::init()`/`shutdown()` recusando um segundo `init()` enquanto o primeiro ainda viver, e tornando o `Rml::Shutdown()` incondicional acima condicional a ser a ÚLTIMA instância viva) é o `UILAYER-SINGLETON-GUARD`, item da W23 -- uma mudança de comportamento que merece fatia própria com prova de mutação própria, conforme a própria regra desta onda de que crescimento no meio da onda vira item da próxima, não desta. O racional de entregar só o aviso agora: um doc dizendo "não suportado" é VERDADEIRO e não carrega risco nenhum; código prometendo um guard que não tem repetiria, dentro deste mesmo item, exatamente a classe de defeito que esta onda passou uma noite caçando em outro lugar do código (uma promessa em prosa que o código não cumpre).

**O próprio caso do consumidor, verificado no repo dele, e é mais seguro do que ele reportou de início.** O caso mais provável de coexistência no mundo real -- um menu de pausa abrindo uma tela de save/load aninhada, cada uma apoiada no próprio `UiLayer` -- está seguro por desenho ESTRUTURAL, não por disciplina de ordenação. `screen_state.hpp` (GusWorld) documenta que `run_screen_state(pai)` tem de RETORNAR (rodando `pai.exit()`, que libera o `UiLayer` do pai) antes que qualquer código possa chamar `run_screen_state(filha)` -- a garantia é a própria sequencialidade de chamada de função do C++, não algo que o autor da tela filha precise lembrar. O consumidor reportou isto como "`reset()` roda primeiro, mais um comentário explica" -- a garantia real é mais forte do que esse enquadramento sugere. Uma nota medida repassada a ele, não um veredito: ele descreveu "seis classes de tela"; uma contagem dos arquivos em `screens/` achou **10** referenciando `UiLayer` e **7** passando por `run_screen_state` -- a diferença (`system_menu_rml`, `save_load_menu_rml`, `battle_cockpit_rml`, `battle_input`) é onde a própria varredura dele da superfície mais ampla deveria começar.

**🔴 ISTO TEM VÍTIMA EM PRODUÇÃO, não é risco teórico (achado pelo GusWorld, 2026-07-30).** Abrir um segundo `UiLayer` com um primeiro ainda vivo crashou DE VERDADE, ao vivo: `screen_state.hpp` do consumidor documenta que isso aconteceu com o log do RmlUi `"Element meta pool not empty on shutdown, 75 object(s) leaked"`, rastreado a um commit de revert no histórico dele (um menu de pausa aberto dentro de uma tela de batalha, revertido no mesmo dia em que entrou, 2026-07-05). A string foi confirmada no RmlUi pinado que de fato linkamos (`ElementMeta.cpp`), e o texto completo da mensagem é: *"Element meta pool not empty on shutdown, %d object(s) leaked. This will likely lead to a crash when element is destroyed. Ensure that no Rml::Element objects are kept alive in user space at the end of Rml::Shutdown."*

**⚠️ O ACHADO QUE VALE MAIS QUE O SINTOMA: A MENSAGEM DO UPSTREAM DESVIA A DIREÇÃO.** Ela manda o consumidor *"garantir que nenhum `Rml::Element` seja mantido vivo em espaço de usuário"* -- culpa o usuário por segurar `Element`s. No mecanismo real descrito acima, ninguém segurou nada: o segundo `Initialise()` trocou o `core_data`, e os 75 objetos ficaram órfãos de um `Context` que o estado global do RmlUi já não conhece mais. **Quem seguir o conselho da mensagem vai caçar vazamento no próprio código e não vai achar nada.** Um sintoma vago faz alguém procurar em todo lugar; um sintoma com direção ERRADA faz alguém procurar com afinco no lugar errado -- e aí a própria competência da pessoa trabalha contra ela: quanto mais rigorosa a auditoria do próprio código, mais confiante ela fica de que o problema não está ali, e mais tempo leva até desconfiar da premissa real. Deslocado de duas formas ao mesmo tempo: no TEMPO (a mensagem dispara no teardown, longe da chamada de construtor que de fato causou, depois de tudo parecer ter funcionado) -- "a mensagem não é o defeito, é o eco dele no teardown" -- e no ESPAÇO, apontando pro código errado inteiramente.

**⚠️ SE VOCÊ VIR ESTA MENSAGEM:** a instrução dela vai mandar você procurar `Rml::Element`s vivos no seu próprio código. **NÃO FAÇA isso primeiro** -- confirme antes que não há um segundo `UiLayer`/`App` vivo no mesmo processo. Só depois de descartar essa hipótese vale auditar o próprio código atrás de um `Element` retido.

**⚠️ REDAÇÃO OBRIGATÓRIA para esta classe de aviso (proposta do próprio consumidor, aceita):** não escrever "o sintoma é X, a causa é Y" -- escrever **"o sintoma é X, ele vai te mandar fazer Z, NÃO FAÇA, confirme Y primeiro"** (exatamente a instrução no parágrafo acima) -- porque quem chega neste ponto já está com a mensagem desviante na tela, e a primeira coisa de que precisa é **permissão para não seguir o conselho dela**, não só a explicação correta em algum lugar mais abaixo. Um sintoma vago faz alguém procurar em todo lugar; um sintoma com direção ERRADA faz alguém procurar com afinco no lugar errado -- e aí a própria competência da pessoa trabalha contra ela: quanto mais rigorosa a auditoria do próprio código, mais confiante ela fica de que o problema não está ali, e mais tempo leva até desconfiar da premissa real. Mais uma consequência do mesmo achado de assert no-op desta seção: logo depois daquela linha de log, `ElementMeta.cpp` tem um `RMLUI_ERROR` que o upstream claramente pretendia usar pra abortar ali -- também no-op nos nossos builds, então o crash que o upstream esperava ser barulhento fica silencioso aqui também.

**Nota de rigor ao citar terceiros.** Uma referência corroborante que o consumidor deu inicialmente (arquivo:linha do próprio repo dele) já estava MORTA no momento da verificação -- o arquivo tinha andado desde que o comentário original foi escrito, e as linhas citadas hoje não têm relação com o achado. A claim segue verdadeira por outras vias medidas independentemente, mas este doc cita o SINTOMA e o MECANISMO, nunca `arquivo:linha` de um repositório de terceiro que se move -- é exatamente a mesma classe de deriva de citação que travou a manhã inteira desta onda, só que num repo que esta biblioteca nem controla.
...
- [`docs/window-modes.md`](window-modes.md): the full per-platform window-mode transition contract (section 26 only covers what `set_mode` does NOT re-trigger at the GL-context level). / o contrato completo de transição de modo de janela por plataforma (a seção 26 só cobre o que o `set_mode` NÃO re-dispara em nível de contexto GL).
- [ADR-0008](adr/0008-embed-guest-mode.md) (again): the `App`-owns-window vs. `UiLayer`-attaches-to-host-context split that section 26 documents the exact creation order for. / (de novo): a divisão `App`-dono-da-janela vs. `UiLayer`-anexa-ao-contexto-do-host, para a qual a seção 26 documenta a ordem exata de criação.
