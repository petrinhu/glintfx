...
1. When `cfg.load_gl` is `true` (the default): `glx_gl_load()` (`ui_layer.cpp:77`) resolves glintfx's GL function-pointer table against **whatever GL context is current at that exact instant** -- there is no wait, no retry, no deferred resolution.
2. `impl_->engine.attach(&impl_->clock, impl_->w, impl_->h, cfg.font_engine)` (`ui_layer.cpp:80`) -- the SAME `RenderGl3::init()`-then-`Bootstrap::init()` sequence described for `App` above, with the SAME shader-compile-on-construction dependency on a populated function table.
...
1. Quando `cfg.load_gl` é `true` (o padrão): `glx_gl_load()` (`ui_layer.cpp:77`) resolve a tabela de ponteiros de função GL da glintfx contra **o que quer que seja o contexto GL corrente naquele instante exato** -- não há espera, nem retry, nem resolução adiada.
2. `impl_->engine.attach(&impl_->clock, impl_->w, impl_->h, cfg.font_engine)` (`ui_layer.cpp:80`) -- a MESMA sequência `RenderGl3::init()`-depois-`Bootstrap::init()` descrita para o `App` acima, com a MESMA dependência de compilação-de-shader-na-construção contra uma tabela de função populada.
...
**EN:** `App::capture_frame()` shipped `FRAMEGRAB-TEX` for `glintfx::App` only (section header of `app.hpp`'s own `CapturedFrame` doc-comment) -- the embed-only build (`GLINTFX_BACKEND_GLFW=OFF`) had zero access to it, and the consumer who asked for the capability (GusWorld) is embed-only. `UiLayer::capture_frame()` (`ui_layer.hpp:900`) closes that gap: same struct shape, same pixel-format/ownership/fail-high contract as `App::CapturedFrame` (`ui_layer.hpp:892`), reading the SAME shared readback both facades wrap, `Engine::capture_frame` (`engine.hpp:202`) -- implemented at `engine.cpp:573`. `App::capture_frame()` (`app.cpp:476`) was itself refactored to delegate to it instead of duplicating the `glReadPixels`/flip/RGBA8-expansion body inline, so the two facades cannot drift.
...
**Fail-high.** `!ok()`, or a degenerate current viewport (`w<=0||h<=0`), returns `CapturedFrame{}` (`ok=false`) without touching GL. `set_viewport()`'s own guard rejects `w<=0||h<=0` in normal use (keeps the previous viewport), so this branch is NOT reachable that way -- but `UiLayerConfig` itself is not validated by the constructor before `Engine::attach()`, so `UiLayerConfig{.logical_width = 0, ...}` reaches `Rml::CreateContext("main", {0, h})` unchecked and DOES leave `ok() == true` (measured, not assumed -- RmlUi accepts a zero-width context at creation without crashing) -- `tests/ui_layer_capture_frame_smoke.cpp`'s own Part G exercises exactly this path, added after the team-lead flagged (in the same fatia's own review thread) that the equivalent `App`-side guard had never fired live under Xvfb (no window manager to minimize a real window to 0x0).
...
**Fail-high.** `!ok()`, ou um viewport corrente degenerado (`w<=0||h<=0`), retorna `CapturedFrame{}` (`ok=false`) sem tocar GL. O próprio guard de `set_viewport()` rejeita `w<=0||h<=0` em uso normal (mantém o viewport anterior), então este ramo NÃO é alcançável por esse caminho -- mas o próprio `UiLayerConfig` não é validado pelo construtor antes de `Engine::attach()`, então `UiLayerConfig{.logical_width = 0, ...}` alcança `Rml::CreateContext("main", {0, h})` sem checagem e DEIXA `ok() == true` (medido, não presumido -- o RmlUi aceita um contexto de largura zero na criação sem crashar) -- a própria Parte G de `tests/ui_layer_capture_frame_smoke.cpp` exercita exatamente este caminho, somada depois que o team-lead sinalizou (na própria thread de review desta fatia) que o guard equivalente do lado `App` nunca disparou ao vivo sob Xvfb (sem window manager pra minimizar uma janela real a 0x0).
