...
**Fail-high.** `!ok()`, or a degenerate current viewport (`w<=0||h<=0` -- not reachable through `set_viewport()`'s own guard in normal use), returns `CapturedFrame{}` (`ok=false`) without touching GL.
...
**Fail-high.** `!ok()`, ou um viewport corrente degenerado (`w<=0||h<=0` -- não alcançável através do próprio guard de `set_viewport()` em uso normal), retorna `CapturedFrame{}` (`ok=false`) sem tocar GL.
