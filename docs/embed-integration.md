...
**EN:** `App::set_frame_callback(std::function<void(float dt_seconds)> cb)` (`glintfx/include/glintfx/app.hpp:507`) fires once per `render()`/`run()` iteration, with the GL context current, AFTER the frame clear and BEFORE the UI render pass -- see `Engine::render_standalone`'s doc-comment (`glintfx/src/engine.hpp:91-126`) for the exact insertion point and the full GL-state save/restore contract. This section adds one thing that comment does not cover: **hosting a complete, independent GL renderer inside `cb` is a supported, validated pattern**, not just a place for a couple of draw calls.
...
**PT:** `App::set_frame_callback(std::function<void(float dt_seconds)> cb)` (`glintfx/include/glintfx/app.hpp:507`) dispara uma vez por iteração de `render()`/`run()`, com o contexto GL corrente, APÓS o clear do frame e ANTES do passe de render de UI -- ver o doc-comment de `Engine::render_standalone` (`glintfx/src/engine.hpp:91-126`) para o ponto de inserção exato e o contrato completo de salvar/restaurar estado GL. Esta seção acrescenta uma coisa que aquele comentário não cobre: **hospedar um renderer GL completo e independente dentro de `cb` é um padrão suportado e validado**, não só um lugar pra algumas chamadas de desenho.
...
| :--- | :--- | :--- |
| `is_key_down(Key k)` (`app.hpp:344`) | `bool` | Level state -- true while the physical key stays held. GLFW's own auto-repeat does not re-set it (already true since the initial press). `false` when `!ok()` or `k` is out of range. |
| `is_mouse_button_down(int button)` (`app.hpp:360`) | `bool` | `button` in `[0, 7]` -- GLFW's own honest ceiling (`GLFW_MOUSE_BUTTON_LAST == 7`), **not** the UI route's 3-button contract (unchanged). Out-of-range (including negative) returns `false`. |
| `get_cursor_pos(float& x, float& y)` (`app.hpp:368`) | `void` | Framebuffer physical pixels, top-left origin, y-down -- same space as `UiEvent::MouseMove`/`get_element_box`. `(0, 0)` when `!ok()` or before any cursor-position event. |
| `set_key_callback(fn(Key key, KeyAction action, int modifiers))` (`app.hpp:381`) | `void` | Edge-detected: fires once per physical Press/Release, once per OS auto-repeat tick (`KeyAction::Repeat`). Fires from inside `poll_events()` (copy-before-invoke). Null/empty `cb` is a safe no-op. |
...
| Method | Returns | Notes |
| :--- | :--- | :--- |
| `request_close()` (`app.hpp:400`) | `void` | Programmatic quit -- BYPASSES the veto below entirely (an intentional act by the code that owns the loop, e.g. a confirmed "Sair" menu item; routing it through the veto would deadlock a confirmation dialog against its own confirm action). |
| `set_close_request_callback(fn() -> bool)` (`app.hpp:413`) | `void` | Veto for a USER-initiated close (X button, Alt+F4) only -- `false` keeps the window open (e.g. show "save before exit?"); any other outcome, or no callback registered, lets the close proceed (matches pre-`HOSTIN-4` behaviour). NEVER fires for `request_close()`. |
| `set_window_focus_callback(fn(bool focused))` / `is_window_focused()` (`app.hpp:432`/`434`) | `void` / `bool` | WINDOW-level focus (OS has-input-focus-at-all), prefixed `window_` so it never collides with the pre-existing DOM `set_focus_callback`. Typical use: auto-pause/duck audio on alt-tab. |
| `set_window_iconify_callback(fn(bool iconified))` / `is_window_iconified()` (`app.hpp:433`/`435`) | `void` / `bool` | Minimize observation, same prefix rationale. Fail-high (`false`) when `!ok()`. |
...
| :--- | :--- | :--- |
| `is_key_down(Key k)` (`app.hpp:344`) | `bool` | Estado de nível -- true enquanto a tecla física continuar segurada. O próprio auto-repeat do GLFW não re-seta (já era true desde o pressionar inicial). `false` quando `!ok()` ou `k` fora de faixa. |
| `is_mouse_button_down(int button)` (`app.hpp:360`) | `bool` | `button` em `[0, 7]` -- o próprio teto honesto do GLFW (`GLFW_MOUSE_BUTTON_LAST == 7`), **não** o contrato de 3 botões da rota de UI (inalterado). Fora de faixa (inclusive negativo) retorna `false`. |
| `get_cursor_pos(float& x, float& y)` (`app.hpp:368`) | `void` | Pixels físicos de framebuffer, origem superior-esquerda, y pra baixo -- mesmo espaço de `UiEvent::MouseMove`/`get_element_box`. `(0, 0)` quando `!ok()` ou antes de qualquer evento de posição de cursor. |
| `set_key_callback(fn(Key key, KeyAction action, int modifiers))` (`app.hpp:381`) | `void` | Edge-detectado: dispara uma vez por Press/Release físico, uma vez por tick de auto-repeat do SO (`KeyAction::Repeat`). Dispara de dentro de `poll_events()` (cópia-antes-de-invocar). `cb` nulo/vazio é no-op seguro. |
...
| Método | Retorna | Notas |
| :--- | :--- | :--- |
| `request_close()` (`app.hpp:400`) | `void` | Saída programática -- BYPASSA o veto abaixo inteiramente (um ato intencional do código dono do loop, ex.: um item de menu "Sair" confirmado; roteá-lo pelo veto travaria um diálogo de confirmação contra a própria ação de confirmar). |
| `set_close_request_callback(fn() -> bool)` (`app.hpp:413`) | `void` | Veto só pra close INICIADO PELO USUÁRIO (botão X, Alt+F4) -- `false` mantém a janela aberta (ex.: mostrar "salvar antes de sair?"); qualquer outro resultado, ou nenhum callback registrado, deixa o close prosseguir (bate com o comportamento pré-`HOSTIN-4`). NUNCA dispara pra `request_close()`. |
| `set_window_focus_callback(fn(bool focused))` / `is_window_focused()` (`app.hpp:432`/`434`) | `void` / `bool` | Focus de nível JANELA (o SO ter foco de input de todo), prefixado `window_` pra nunca colidir com o `set_focus_callback` de DOM pré-existente. Uso típico: auto-pausar/abafar áudio no alt-tab. |
| `set_window_iconify_callback(fn(bool iconified))` / `is_window_iconified()` (`app.hpp:433`/`435`) | `void` / `bool` | Observação de minimizar, mesmo racional de prefixo. Fail-high (`false`) quando `!ok()`. |
...
1. **`window.create(cfg.title, cfg.width, cfg.height, initial_mode)`** (`WindowGlfw::create`, `glintfx/src/window_glfw.cpp:53-220`) -- validates `window_mode` first (an invalid enum falls back to `Windowed`, `app.cpp:128-138`), then requests the window: `glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3)`, `GLFW_CONTEXT_VERSION_MINOR, 3)`, `GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE)` (`window_glfw.cpp:82-84`) -- **a GL 3.3 core-profile context is what glintfx always requests; `AppConfig` has no field to change this.** MSAA is disabled process-wide via `RMLUI_NUM_MSAA_SAMPLES=0` (`glintfx/CMakeLists.txt:1416`, already covered in section 3). Then `glfwCreateWindow(...)` (`window_glfw.cpp:174`) and `glfwMakeContextCurrent(win_)` (`window_glfw.cpp:183`) -- the context becomes current, on the calling thread, before anything else in `create()` runs -- and finally `glx_gl_load()` (`window_glfw.cpp:184`), glintfx's own GL 3.3 loader, resolving its function-pointer table against that now-current context. `create()` returns `false` if either `glfwCreateWindow` or `glx_gl_load()` fails (`window_glfw.cpp:175`, `184-188`), and `App::ok()` reflects this all the way up (`app.cpp:139`: `if (!impl_->window.create(...)) return;` -- the constructor returns early, `impl_->ok` stays `false`).
...
1. **`window.create(cfg.title, cfg.width, cfg.height, initial_mode)`** (`WindowGlfw::create`, `glintfx/src/window_glfw.cpp:53-220`) -- valida `window_mode` primeiro (um enum inválido cai pra `Windowed`, `app.cpp:128-138`), depois pede a janela: `glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3)`, `GLFW_CONTEXT_VERSION_MINOR, 3)`, `GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE)` (`window_glfw.cpp:82-84`) -- **um contexto GL 3.3 core-profile é o que a glintfx sempre pede; `AppConfig` não tem campo para mudar isto.** MSAA é desabilitado em nível de processo via `RMLUI_NUM_MSAA_SAMPLES=0` (`glintfx/CMakeLists.txt:1416`, já coberto na seção 3). Depois `glfwCreateWindow(...)` (`window_glfw.cpp:174`) e `glfwMakeContextCurrent(win_)` (`window_glfw.cpp:183`) -- o contexto fica corrente, na thread chamadora, antes de qualquer outra coisa em `create()` rodar -- e por fim `glx_gl_load()` (`window_glfw.cpp:184`), o próprio loader GL 3.3 da glintfx, resolvendo a tabela de ponteiros de função contra esse contexto agora corrente. `create()` retorna `false` se `glfwCreateWindow` OU `glx_gl_load()` falhar (`window_glfw.cpp:175`, `184-188`), e `App::ok()` reflete isso até o topo (`app.cpp:139`: `if (!impl_->window.create(...)) return;` -- o construtor retorna cedo, `impl_->ok` fica `false`).
...
**EN:** `App::set_window_mode`/`WindowGlfw::set_mode` (section on window modes, `docs/window-modes.md`) reuses the SAME GLFW window and context across a mode switch: `glfwSetWindowMonitor(...)` (`window_glfw.cpp:334`, `:355`, `:382`, `:386`) is the only window-system call `set_mode()` makes -- there is no `glfwCreateWindow`, no `glfwMakeContextCurrent`, no `glx_gl_load()` call anywhere in `WindowGlfw::set_mode()` (`window_glfw.cpp:311-401`, read in full). A runtime fullscreen/maximize/windowed toggle (e.g. an options-menu Alt+Enter) therefore never re-triggers the creation sequence above -- the GL context and its loaded function table stay the SAME ones `App::App()`'s original `create()` call produced, for the entire life of the `App` instance.

**PT:** `App::set_window_mode`/`WindowGlfw::set_mode` (seção de modos de janela, `docs/window-modes.md`) reutiliza a MESMA janela e contexto GLFW através de uma troca de modo: `glfwSetWindowMonitor(...)` (`window_glfw.cpp:334`, `:355`, `:382`, `:386`) é a única chamada ao sistema de janelas que `set_mode()` faz -- não há `glfwCreateWindow`, nem `glfwMakeContextCurrent`, nem chamada a `glx_gl_load()` em lugar nenhum de `WindowGlfw::set_mode()` (`window_glfw.cpp:311-401`, lido por inteiro). Um toggle de fullscreen/maximizar/windowed em runtime (ex.: Alt+Enter num menu de opções) portanto nunca re-dispara a sequência de criação acima -- o contexto GL e a tabela de função carregada continuam sendo os MESMOS que a chamada `create()` original de `App::App()` produziu, pela vida inteira da instância `App`.
