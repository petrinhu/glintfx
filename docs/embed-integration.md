...
**Estado GL.** Salvo e restaurado automaticamente -- mas NÃO via o `GlStateGuard` que o próprio contrato compose-only do `render()` usa (seção 4 acima): um readback de pixel puro toca 5 peças de estado GL DIFERENTES (binding de `GL_READ_FRAMEBUFFER`, `GL_READ_BUFFER`, `GL_PACK_ALIGNMENT`, `GL_PACK_ROW_LENGTH`, binding de `GL_PIXEL_PACK_BUFFER`), nenhuma das quais o guard do passe de render captura. `GL_PACK_ALIGNMENT` é forçado a `1` pra própria leitura -- o conserto pra uma classe de bug de corrupção de heap real e medida: no alinhamento default do GL de `4`, uma leitura RGB de largura ímpar (3 bytes/pixel) preenche cada linha até o próximo limite de 4 bytes, o que o próprio `glReadPixels` leva em conta internamente mas um buffer de destino compactado dimensionado `width*height*3` não tem espaço para -- confirmado revertendo deliberadamente o alinhamento forçado durante o próprio review desta fatia e rebuildando: o teste de largura ímpar (`tests/ui_layer_capture_frame_smoke.cpp`, Parte F) crashou com `double free or corruption (!prev)`, não meramente pixels errados.
