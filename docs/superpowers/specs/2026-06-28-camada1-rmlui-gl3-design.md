...
- Distribuída como **um único alvo CMake**, consumível via `FetchContent` (e/ou `find_package`/`add_subdirectory`), trazendo RmlUi + gl3w + renderer GL3 + GLFW **empacotados** (batteries-included). O consumidor faz só `target_link_libraries(app PRIVATE glintfx)` — **nada de wirar GL/janela/loader à mão**.
...
glintfx/        Camada 1 (C++23) — a lib
  include/glintfx/  headers públicos da fachada (<glintfx/glintfx.hpp>)
  src/          M0–M3
  demos/        showcase.rml/.rcss + main do demo
  CMakeLists.txt
examples/       upstreams clonados (gitignored): RmlUi, gl3w
```
Nome da lib: **glintfx** (namespace `glintfx::`, header `<glintfx/glintfx.hpp>`, alvo CMake `glintfx`).
