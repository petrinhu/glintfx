...
| 2026-08-05 | a | 🔴 **`S6a`'s `HEAD` scan is comment-unaware, and picks up prose from an XML comment as if it were the `<head>` payload.** `dom_dump_scan_head()` (`glintfx/src/rml/dom_dump.cpp`) takes the FIRST literal `<head` in the source; in 15 of the corpus's 16 fixtures that occurrence is inside the leading `<!-- ... -->` provenance comment (the sentence *"its own `<head>`-opacity raw-byte-scan..."*), ~819-1069 source bytes ahead of the real `<head>` element. `S6b` skips comments and starts at the real element. Both stop at the same `</head`, so the shapes nest exactly: the rml payload is `<extra prefix> + <the uix payload>`, 829-1083 EXTRA escaped bytes depending on the fixture. **Only the `HEAD` line differs** -- on every one of the 16 fixtures every other line is byte-identical, i.e. the two independently written trees agree completely on structure, ordering, escaping, classes, attributes and text. **Control that turns this from an assertion into a measurement:** `gusworld_battle_cockpit.rml`, the single fixture whose comment contains no literal `<head`, is byte-IDENTICAL end to end. **Why class (a) and not (b):** RmlUi's own parser ignores the comment correctly -- nothing upstream normalizes anything here; section 4 defines the payload from "the `>` that closes the `<head>` open tag", and a `<head>` inside a comment is not an open tag, so this is `S6a` not implementing section 4. ⚠️ **Why it was not caught earlier:** `dom_dump.hpp`'s own header states no corpus fixture exercises a `</head` collision -- true, but it enumerated only TWO of the three corpus directories (`glintfx/tests/`, `glintfx/demos/showcase/`); the third, `glintfx/src/uix/dom/test_fixtures/`, added later by `S3`, is exactly where the collision lives, and it is on the OPENING `<head`, not the closing one. | 15 of the 16 fixtures in `glintfx/src/uix/dom/test_fixtures/` -- every one except `gusworld_battle_cockpit.rml` | **Measured, deliberately NOT fixed by the slice that found it.** `S7`'s product is the oracle plus this row; editing a dumper to flatten a difference the oracle just found would turn the measurement into a tautology (the exact failure the two-independent-authors design exists to prevent). PINNED by mechanism -- strict-suffix shape + measured extra-prefix length + verbatim prefix head -- in `glintfx/src/rml/dom_dump_differential_oracle.cpp`'s `kKnownDivergences`, which fails both when the divergence changes shape and when it silently disappears. The fix belongs to a named follow-up slice over `dom_dump_scan_head()`. ⚠️ **Proposed and NOT applied here:** section 4 should say explicitly that the `<head>` open tag is the document's real element, and that a literal `<head`/`</head` inside a comment, a quoted attribute value, or CSS/JS text is not one -- both dumpers use a raw byte scan for this payload, so the spec owes both implementers that warning. That edits sections 1-8, which this section's own carve-out puts behind the líder's sign-off; it is therefore raised, not done. |
...
| 2026-08-05 | a | 🔴 **O scan de `HEAD` da `S6a` não tem consciência de comentário, e pega prosa de dentro de um comentário XML como se fosse o payload do `<head>`.** O `dom_dump_scan_head()` (`glintfx/src/rml/dom_dump.cpp`) pega o PRIMEIRO `<head` literal da fonte; em 15 das 16 fixtures do corpus essa ocorrência está dentro do comentário de proveniência `<!-- ... -->` inicial (a frase *"its own `<head>`-opacity raw-byte-scan..."*), ~819-1069 bytes-fonte à frente do elemento `<head>` real. A `S6b` pula comentários e começa no elemento real. Os dois param no mesmo `</head`, então as formas encaixam exatamente: o payload rml é `<prefixo extra> + <o payload uix>`, 829-1083 bytes escapados A MAIS conforme a fixture. **Só a linha `HEAD` difere** -- em cada uma das 16 fixtures toda outra linha é byte-idêntica, ou seja, as duas árvores escritas de forma independente concordam por completo em estrutura, ordenação, escape, classes, atributos e texto. **Controle que transforma isto de afirmação em medição:** a `gusworld_battle_cockpit.rml`, a única fixture cujo comentário não contém `<head` literal, é byte-IDÊNTICA de ponta a ponta. **Por que classe (a) e não (b):** o próprio parser do RmlUi ignora o comentário corretamente -- não há normalização nenhuma upstream aqui; a seção 4 define o payload a partir do "`>` que fecha a tag de abertura `<head>`", e um `<head>` dentro de comentário não é tag de abertura, logo é a `S6a` não implementando a seção 4. ⚠️ **Por que não foi pego antes:** o próprio cabeçalho do `dom_dump.hpp` declara que nenhuma fixture do corpus exercita colisão de `</head` -- verdade, mas ele enumerou só DOIS dos três diretórios de corpus (`glintfx/tests/`, `glintfx/demos/showcase/`); o terceiro, `glintfx/src/uix/dom/test_fixtures/`, somado depois pela `S3`, é exatamente onde a colisão mora, e ela é no `<head` de ABERTURA, não no de fechamento. | 15 das 16 fixtures em `glintfx/src/uix/dom/test_fixtures/` -- todas exceto a `gusworld_battle_cockpit.rml` | **Medida, deliberadamente NÃO consertada pela fatia que a achou.** O produto da `S7` é o oráculo mais esta linha; editar um dumper pra achatar uma diferença que o oráculo acabou de achar transformaria a medição numa tautologia (exatamente a falha que o desenho de dois-autores-independentes existe pra prevenir). PINADA por mecanismo -- forma de sufixo estrito + comprimento medido do prefixo extra + cabeça do prefixo verbatim -- no `kKnownDivergences` do `glintfx/src/rml/dom_dump_differential_oracle.cpp`, que reprova tanto quando a divergência muda de forma quanto quando ela some em silêncio. O conserto pertence a uma fatia nomeada de acompanhamento sobre o `dom_dump_scan_head()`. ⚠️ **Proposto e NÃO aplicado aqui:** a seção 4 deveria dizer explicitamente que a tag de abertura `<head>` é o elemento real do documento, e que um `<head`/`</head` literal dentro de comentário, de valor de atributo entre aspas, ou de texto CSS/JS não é uma -- os dois dumpers usam scan de byte cru pra este payload, então a spec deve esse aviso aos dois implementers. Isso edita as seções 1-8, que o carve-out desta própria seção põe atrás do aval do líder; portanto está levantado, não feito. |
