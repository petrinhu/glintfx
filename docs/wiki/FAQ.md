...
One repository, two independent tracks ("layers"): [glintfx](Layer-1-glintfx) (Layer 1, released, active product) and [loucura_c_asm](Layer-0-Core) (Layer 0, experimental, long-term). They do not link into each other; see [ADR-0006](https://github.com/petrinhu/glintfx/blob/main/docs/adr/0006-layered-hybrid-architecture.md).
...
Not today. glintfx is pure, ordinary C++ that links normal system libraries. Layer 0 is a separate, unreleased experiment. Per [ADR-0009](https://github.com/petrinhu/glintfx/blob/main/docs/adr/0009-internalization-boundary.md), the long-term plan is a narrow, incremental static link of specific pieces, not a wholesale merge.

**What platforms are supported?**
Linux on x86-64 only, for both layers. No macOS, no ARM. See the "Known limitations" section of the main [`README.md`](https://github.com/petrinhu/glintfx/blob/main/README.md). (On Windows specifically, see the next question -- there is a real but narrow exception.)
...
Um repositório, duas trilhas independentes ("camadas"): [glintfx](Layer-1-glintfx) (Camada 1, lançada, produto ativo) e [loucura_c_asm](Layer-0-Core) (Camada 0, experimental, longo prazo). Elas não se linkam uma na outra; ver [ADR-0006](https://github.com/petrinhu/glintfx/blob/main/docs/adr/0006-layered-hybrid-architecture.md).
...
Não hoje. O glintfx é C++ puro e comum, que linka bibliotecas de sistema normais. A Camada 0 é um experimento separado e não lançado. Pela [ADR-0009](https://github.com/petrinhu/glintfx/blob/main/docs/adr/0009-internalization-boundary.md), o plano de longo prazo é um link estático estreito e incremental de peças específicas, não uma fusão total.

**Quais plataformas são suportadas?**
Só Linux em x86-64, pras duas camadas. Sem macOS, sem ARM. Ver a seção "Limitações conhecidas" do [`README.md`](https://github.com/petrinhu/glintfx/blob/main/README.md) principal. (No Windows especificamente, ver a próxima pergunta -- há uma exceção real, mas estreita.)
...
Sim. Desde a `v0.10.0`, o motor de fonte próprio clean-room da glintfx é o default (`FontEngine::Own`), mas o motor apoiado em FreeType do RmlUi continua plenamente linkado e a uma linha de config de distância: defina `font_engine = glintfx::FontEngine::FreeType` em `AppConfig`/`UiLayerConfig` antes da construção -- sem rebuild. Ver [ADR-0011](https://github.com/petrinhu/glintfx/blob/main/docs/adr/0011-soft-font-flip.md).
