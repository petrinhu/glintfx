...
**Status:** implementação completa (bootstrap, harness de teste próprio, uma "libc"-núcleo pequena de memória/string/conversão de número, um mini-printf, um alocador bump de memória via `mmap`, mais um parser TrueType/OpenType `SOV-SFNT` clean-room e um rasterizador anti-aliased `SOV-RAST` -- a fundação de uma internalização do FreeType), taggeada `core-v0.3.0`, e agora aguardando uma auditoria externa final antes de todo item ser marcado como totalmente verificado. Esta é uma **trilha de longo prazo, sem prazo**, não um produto com usuários ainda.
