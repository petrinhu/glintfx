...
**Status:** implementação completa (bootstrap, harness de teste próprio, uma "libc"-núcleo pequena de memória/string/conversão de número, um mini-printf, um alocador bump de memória via `mmap`), taggeada `core-v0.1.0`, e agora aguardando uma auditoria externa final antes de todo item ser marcado como totalmente verificado. Esta é uma **trilha de longo prazo, sem prazo**, não um produto com usuários ainda.

### Por que existe

Esta é a fundação de longo prazo da ambição de "internalização" do projeto: ao longo de anos, pedaços das dependências de terceiros da [Camada 1 (glintfx)](Layer-1-glintfx) (RmlUi, FreeType, GLFW) podem ser reconstruídos clean-room em cima deste runtime soberano, onde valer o esforço. Uma peça já fez essa jornada (o loader próprio de OpenGL do glintfx). Ver [ADR-0009](https://codeberg.org/petrinhu/glintfx/src/branch/main/docs/adr/0009-internalization-boundary.md) pra fronteira exata e honesta do que será (e não será) internalizado algum dia: o driver de GPU e o próprio kernel estão explicitamente fora de escopo, pra sempre.
