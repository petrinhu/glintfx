...
Run these commands from any machine with push access to the repository's GitHub remote. **This step is a remote-visible action (a push) and, per this project's own working rules, requires the project lead's explicit authorization before running.** Do not run it as a routine follow-up to a documentation commit.

```sh
# 1. Clone the wiki repo (note the .wiki.git suffix, a URL distinct from the main repo)
git clone https://github.com/petrinhu/glintfx.wiki.git glintfx-wiki
cd glintfx-wiki
...
O GitHub implementa a funcionalidade de Wiki como um repositório git comum, vivendo numa URL fixa ao lado da principal: `https://github.com/<dono>/<repo>.wiki.git`. Cloná-lo, editar arquivos e dar push é idêntico a trabalhar com qualquer outro repositório git; não há exigência especial de edição só-pela-web. Por ser um **histórico git separado**, os próprios commits deste repositório (incluindo o que adicionou esta pasta) nunca o tocam diretamente: uma pessoa (ou um agent autorizado) precisa clonar e dar push explicitamente.
...
Rode estes comandos de qualquer máquina com acesso de push ao remoto GitHub do repositório. **Este passo é uma ação visível em remoto (um push) e, pelas regras de trabalho deste projeto, exige autorização explícita do líder antes de rodar.** Não rode como consequência de rotina de um commit só-de-documentação.
...
git push origin master   # wikis do GitHub usam "master"; confirme com `git branch --show-current` depois do clone
```

### Mantendo sincronizado depois

Não há automação ligando `docs/wiki/` deste repositório à Wiki ao vivo (deliberadamente: um push de Wiki é uma ação em remoto que exige autorização explícita, não algo que o CI deveria fazer sem supervisão). Quando uma mudança futura atualizar `docs/wiki/*.md` num commit normal, rerode os passos de `cp` + `git commit` + `git push` acima contra o clone da wiki pra trazê-la de volta à sincronia. Se isso virar uma tarefa frequente, considere propor um job de CI disparado manualmente pra isso, mas essa é uma decisão nova, não algo que este documento autoriza por conta própria.
