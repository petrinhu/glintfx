...
//     render_gl3.cpp -- moved here VERBATIM (comments preserved) behind the FxHook seam declared
//     in ../fx_hook.hpp. See that header's own file comment for the module-gate mechanism and
//     the fx=ON/OFF handle-uniformity invariant this file is the sole owner of upholding.
// PT: FX-CARVE-1 -- implementação do módulo fx: os programas GLSL "glintfx-tint" (ADR-0010) e
//     "glintfx-ripple" (ADR-0012, L1.22-WAVE/L1.22-CAPTURE), o despacho deles de CompileShader/
//     RenderShader/ReleaseShader, e a maquinaria de captura de backdrop, tudo antes fundido
//     dentro de render_gl3.cpp -- movido pra cá LITERALMENTE (comentários preservados) atrás da
//     costura FxHook declarada em ../fx_hook.hpp. Ver o próprio comentário de cabeçalho daquele
//     header pro mecanismo de gate do módulo e o invariante de uniformidade-de-handle fx=ON/OFF
//     do qual este arquivo é o único dono de manter.

#include "../fx_hook.hpp"

// EN: gl_loader.h FIRST — defines all GL 3.x function pointers loaded at runtime (same ordering
...
//     ri.RenderInterface_GL3::CompileShader/RenderShader/ReleaseShader (qualified, non-virtual
//     calls, ADR-0010 Decision (a)) and ri.GetTransform()/ri.ResetProgram().
// PT: Header do backend do RmlUi a partir de rmlui_SOURCE_DIR/Backends (via CMake) -- dá a
//     definição completa de RenderInterface_GL3 de que este arquivo precisa pra chamar
//     ri.RenderInterface_GL3::CompileShader/RenderShader/ReleaseShader (chamadas qualificadas,
//     não-virtuais, Decisão (a) do ADR-0010) e ri.GetTransform()/ri.ResetProgram().
#include "RmlUi_Renderer_GL3.h"

// EN: GLINTFX-TINT-2 — Dictionary/Variant (Rml::Get, CompileShader parameters) and Log, for the
//     "glintfx-tint"/"glintfx-ripple" CompileShader/RenderShader/ReleaseShader dispatch below.
//     See ADR-0010 and decorator_image_tint.{hpp,cpp} for the full design.
// PT: GLINTFX-TINT-2 — Dictionary/Variant (Rml::Get, parâmetros de CompileShader) e Log, para o
//     despacho "glintfx-tint"/"glintfx-ripple" de CompileShader/RenderShader/ReleaseShader
//     abaixo. Ver ADR-0010 e decorator_image_tint.{hpp,cpp} para o design completo.
#include <RmlUi/Core/Dictionary.h>
#include <RmlUi/Core/Log.h>

#include <cmath>   // EN: std::sqrt (ripple's auto max-radius = captured backdrop diagonal, render_shader).
                   // PT: std::sqrt (raio-max auto do ripple = diagonal do backdrop capturado, render_shader).
#include <memory>  // EN: std::make_unique (CreateGl3FxHook). PT: std::make_unique (CreateGl3FxHook).
#include <vector>
...
//     never touched here). Dispatched via Gl3FxHook::compile_shader/render_shader/
//     release_shader below (class Gl3FxHook, further down this file) — see ADR-0010
//     (docs/adr/0010-image-tint-luminance-key.md) Decision (a) for why the actual GL
//     draw call MUST happen from inside a method holding a live RenderInterface_GL3& (only there
//     is `ri.GetTransform()`/`ri.ResetProgram()` reachable).
//
...
//     (c)). `_transform` is fed directly from ri.GetTransform() (public, ALREADY
...
//     decorator_image_tint.cpp:GenerateElementData, nunca tocada aqui). Despachado por
//     Gl3FxHook::compile_shader/render_shader/release_shader abaixo (classe Gl3FxHook, mais
//     adiante neste arquivo) — ver a Decisão (a) do ADR-0010
//     (docs/adr/0010-image-tint-luminance-key.md) pra por que a chamada de draw GL de fato
//     PRECISA acontecer de dentro de um método com um RenderInterface_GL3& vivo (só lá
//     `ri.GetTransform()`/`ri.ResetProgram()` são alcançáveis).
//
...
//     ri.GetTransform() (público, JÁ projeção×transform pré-combinados —
...
//     rather than duplicated). Dispatched via the SAME Gl3FxHook::compile_shader/render_shader/
//     release_shader as "glintfx-tint" (further down this file), tagged by
...
//     Despachado pelos MESMOS compile_shader/render_shader/release_shader do Gl3FxHook que
//     "glintfx-tint" (mais adiante neste arquivo), marcado por GlintfxShaderHandle::is_ripple.
//
...
//     Dictionary parameters (see ImageTintDecorator::RenderElement) into render_shader's uniforms.
//     `vao`/`index_count` identify the glintfx-owned geometry to draw (decorator_image_tint.cpp's
//     ImageTintElementData) — render_shader's own `geometry_handle` parameter (RmlUi's opaque
...
//     render_shader. `vao`/`index_count` identificam a geometria dona-da-glintfx a desenhar
//     (ImageTintElementData de decorator_image_tint.cpp) — o próprio parâmetro `geometry_handle`
//     de render_shader (o Rml::CompiledGeometryHandle opaco do RmlUi) NUNCA é desreferenciado num
...
//     render_shader's uniforms. `vao`/`index_count` identify the glintfx-owned geometry to draw
//     (decorator_ripple.cpp's RippleElementData) — mirrors GlintfxTintShaderData's own
//     vao/index_count field pair immediately above, same rationale (render_shader's own
...
//     decorator_ripple.hpp's RippleDecorator doc comment); render_shader's ripple branch
//     substitutes `sqrt(cap_w_^2 + cap_h_^2)` (the captured backdrop's own diagonal) whenever
//     `max_radius_arg<=0.f`, since only Gl3FxHook — not decorator_ripple.cpp, which runs
...
//     uniforms do render_shader. `vao`/`index_count` identificam a geometria dona-da-glintfx a
...
//     `geometry_handle` de render_shader também NUNCA é desreferenciado num draw de ripple).
...
//     render_shader substitui por `sqrt(cap_w_^2 + cap_h_^2)` (a própria diagonal do backdrop
//     capturado) sempre que `max_radius_arg<=0.f`, já que só Gl3FxHook — não
...
//     by every Gl3FxHook::compile_shader call — is_tint=false and is_ripple=false for
...
//     compile_shader/render_shader/release_shader below for the unwrap-and-delegate-or-draw split.
// PT: União discriminada retornada (como um Rml::CompiledShaderHandle opaco, via
//     reinterpret_cast) por toda chamada de Gl3FxHook::compile_shader — is_tint=false e
...
//     os três são mutuamente exclusivos. Ver compile_shader/render_shader/release_shader abaixo
//     pra divisão desembrulha-e-delega-ou-desenha.
...
// EN: Gl3FxHook -- FX-CARVE-1's concrete FxHook (../fx_hook.hpp). Owns every GL resource the fx
//     module ever allocates (tint/ripple programs, backdrop-capture texture) -- released in the
//     destructor, same GL-context-still-alive timing the pre-carve Gl3RenderInterface destructor
//     had (this object is a member of Gl3RenderInterface, constructed/destructed alongside it --
//     see render_gl3.cpp's `std::unique_ptr<FxHook> fx_` member).
// PT: Gl3FxHook -- o FxHook concreto do FX-CARVE-1 (../fx_hook.hpp). Dono de todo recurso GL que
//     o módulo fx algum dia aloca (programas tint/ripple, textura de captura de backdrop) --
//     liberado no destrutor, mesmo timing de contexto-GL-ainda-vivo que o destrutor do
//     Gl3RenderInterface tinha antes da carve (este objeto é membro do Gl3RenderInterface,
//     construído/destruído junto dele -- ver o membro `std::unique_ptr<FxHook> fx_` de
//     render_gl3.cpp).
class Gl3FxHook : public FxHook {
public:
  ...
  //     release_shader's doc comment below).
  ...
  //     ver o doc-comment de release_shader abaixo).
  ...
  // -------------------------------------------------------------------------
  // EN: GLINTFX-TINT-2 — compile_shader/render_shader/release_shader. See the
  //     GlintfxShaderHandle/GlintfxTintShaderData doc comments above this class, and ADR-0010
  //     Decision (a), for the full design.
  // PT: GLINTFX-TINT-2 — compile_shader/render_shader/release_shader. Ver os
  ...
  Rml::CompiledShaderHandle compile_shader(RenderInterface_GL3& ri, const Rml::String& name,
    ...
    //     delegate unchanged to the base class (qualified call, ADR-0010 Decision (a)), then wrap
    //     the result too (is_tint=false, is_ripple=false) so render_shader/release_shader can
    //     uniformly unwrap every Rml::CompiledShaderHandle this class ever hands out.
    ...
    //     decorator_polygon.cpp) — delega inalterado pra classe base (chamada qualificada,
    //     Decisão (a) do ADR-0010), depois embrulha o resultado também (is_tint=false,
    //     is_ripple=false) pra que render_shader/release_shader possam desembrulhar
    //     uniformemente todo Rml::CompiledShaderHandle que esta classe já entregou.
    Rml::CompiledShaderHandle base = ri.RenderInterface_GL3::CompileShader(name, parameters);
    if (!base)
      return 0;
    auto* wrapper = new GlintfxShaderHandle{};
  ...
  void render_shader(RenderInterface_GL3& ri, Rml::CompiledShaderHandle shader_handle,
      Rml::CompiledGeometryHandle geometry_handle, Rml::Vector2f translation, Rml::TextureHandle texture) override {
    auto* wrapper = reinterpret_cast<GlintfxShaderHandle*>(shader_handle);
    if (!wrapper)
      return;

    if (!wrapper->is_tint && !wrapper->is_ripple) {
      ri.RenderInterface_GL3::RenderShader(wrapper->base_handle, geometry_handle, translation, texture);
      return;
    }
    ...
    if (wrapper->is_ripple) {
      ...
      //     handle. Same `ri.ResetProgram()`-on-every-exit-path discipline as the tint branch
      ...
      //     `ri.ResetProgram()`-em-todo-caminho-de-saída do ramo de tint abaixo (ver o
      ...
      if (ripple_program_ && wrapper->ripple_data.vao != 0 && wrapper->ripple_data.index_count > 0) {
        ...
        if (backdrop_capture_tex_ != 0) {
          ...
          glUniformMatrix4fv(ripple_loc_transform_, 1, GL_FALSE, ri.GetTransform().data());
    ...
    //     `ri.GetTransform()`/`ri.ResetProgram()` (both public, inherited from
    //     RenderInterface_GL3) are reachable because this draw happens with a live
    //     RenderInterface_GL3& in hand — see ADR-0010 Decision (a). `ri.ResetProgram()` runs on
    ...
    //     `ri.GetTransform()`/`ri.ResetProgram()` (ambos públicos, herdados de
    //     RenderInterface_GL3) são alcançáveis porque este draw acontece com um
    //     RenderInterface_GL3& vivo em mãos — ver a Decisão (a) do ADR-0010. `ri.ResetProgram()`
    //     roda em TODO caminho de saída abaixo (só há um, no fim — nenhum early-return após
    ...
    if (tint_program_ && wrapper->tint_data.vao != 0 && wrapper->tint_data.index_count > 0) {
      ...
      glUniformMatrix4fv(tint_loc_transform_, 1, GL_FALSE, ri.GetTransform().data());
      glActiveTexture(GL_TEXTURE0);
  ...
  void release_shader(RenderInterface_GL3& ri, Rml::CompiledShaderHandle shader_handle) override {
    auto* wrapper = reinterpret_cast<GlintfxShaderHandle*>(shader_handle);
    if (!wrapper)
      return;
    // EN: Delegate-or-free-tag-only split, mirrors compile_shader. The VAO/VBO/EBO
    ...
    // PT: Divisão delega-ou-libera-só-a-tag, espelha compile_shader. A VAO/VBO/EBO
    ...
    if (!wrapper->is_tint && !wrapper->is_ripple)
      ri.RenderInterface_GL3::ReleaseShader(wrapper->base_handle);
    delete wrapper;
  }

  // ---------------------------------------------------------------------------
  // EN: L1.22-CAPTURE, COLD-START FIX (post-647350f QA finding) — split into a cheap "arm" step
  //     (called unconditionally from Gl3RenderInterface::arm_backdrop, render_gl3.cpp, itself
  //     called from RenderGl3::begin_frame_compose) and a real, on-demand "ensure" step (called
  //     from render_shader's ripple branch, above, exactly once per frame, the very first time a
  //     ripple element actually needs to sample the backdrop). This REPLACES the original
  //     counter-gated design (`ripple_active_counter_`, `RenderGl3::set_ripple_active_counter`),
  //     which had a real cold-start bug: the counter (incremented in decorator_ripple.cpp's
  //     GenerateElementData) is only accurate DURING/AFTER Rml::Context::Render()'s tree walk
  //     (confirmed by reading the pinned Source/Core/ElementEffects.cpp: `InstanceEffects()`/
  //     `GenerateElementData` both run from `Element::Render()` -> `RenderEffects()`, i.e. INSIDE
  //     Context::Render(), never earlier), but the old gate read it BEFORE Context::Render() even
  //     started (at the top of begin_frame_compose) -- so on the very FIRST frame a ripple
  //     decorator became active, the counter still read 0 (last frame's value), the capture was
  //     skipped, and the ripple shader sampled a still-zero `backdrop_capture_tex_` -- the
  //     "opening frame" of a `ripple-phase: 0` keyframe animation (the most visually intense one)
  //     silently rendered as nothing. ripple_sanity.cpp (QA, independent of this implementer)
  //     proves this with a spy on the real glCopyTexSubImage2D entry point plus hand-derived
  //     expected pixels.
  //
  //     WHY ON-DEMAND-INSIDE-RENDER_SHADER FIXES IT WITHOUT REINTRODUCING A COUNTER: the decision
  ...
  //     reachable from inside the "glintfx-ripple" render_shader branch, which is only ever
  ...
  //     Context::Render() (from a render_shader call), it is unconditionally BEFORE EndFrame(),
  ...
  //     by arm_backdrop_capture, called once per begin_frame_compose via
  //     Gl3RenderInterface::arm_backdrop) -- if a document has MULTIPLE ripple elements, only the
  //     FIRST one drawn each frame triggers the real GL capture; every later ripple element that
  //     frame reuses the SAME `backdrop_capture_tex_` (still correct: none of them can see each
  //     other's own draws, since RmlUi's own render-layer output never reaches FBO 0 until
  //     EndFrame(), long after every ripple element has already drawn).
  //
  ...
  //     barato (chamado incondicionalmente de Gl3RenderInterface::arm_backdrop, render_gl3.cpp,
  //     ele próprio chamado de RenderGl3::begin_frame_compose) e num passo "ensure" real, sob
  //     demanda (chamado do ramo de ripple de render_shader, acima, exatamente uma vez por
  //     frame, na primeira vez que um elemento ripple de fato precisa amostrar o backdrop). Isto
  //     SUBSTITUI o design original protegido por contador (`ripple_active_counter_`,
  //     `RenderGl3::set_ripple_active_counter`), que tinha um bug real de cold-start: o contador
  //     (incrementado em GenerateElementData de decorator_ripple.cpp) só é preciso DURANTE/DEPOIS
  //     da caminhada de árvore de Rml::Context::Render() (confirmado lendo o
  //     Source/Core/ElementEffects.cpp pinado: `InstanceEffects()`/`GenerateElementData` ambos
  //     rodam a partir de `Element::Render()` -> `RenderEffects()`, isto é, DENTRO de
  //     Context::Render(), nunca antes), mas o gate antigo o lia ANTES de Context::Render()
  //     sequer começar (no topo de begin_frame_compose) -- então no PRIMEIRO frame em que um
  //     decorator ripple ficava ativo, o contador ainda lia 0 (valor do frame anterior), a
  //     captura era pulada, e o shader de ripple amostrava um `backdrop_capture_tex_` ainda zero
  //     -- o frame de "abertura" de uma animação de `ripple-phase: 0` (o mais intenso
  //     visualmente) renderizava silenciosamente como nada. ripple_sanity.cpp (QA, independente
  //     deste implementador) prova isso com um espião no entry point real glCopyTexSubImage2D
  //     mais pixels esperados derivados à mão.
  //
  //     POR QUE SOB-DEMANDA-DENTRO-DO-RENDER_SHADER CONSERTA SEM REINTRODUZIR UM CONTADOR: a
  ...
  //     de dentro do ramo "glintfx-ripple" de render_shader, que só é alcançado se
  ...
  //     de uma chamada de render_shader), está incondicionalmente ANTES de EndFrame(), então o
  ...
  //     false por arm_backdrop_capture, chamado uma vez por begin_frame_compose via
  //     Gl3RenderInterface::arm_backdrop) -- se um documento tem MÚLTIPLOS elementos ripple, só o
  //     PRIMEIRO desenhado a cada frame dispara a captura GL de fato; todo elemento ripple
  //     posterior naquele frame reusa o MESMO `backdrop_capture_tex_` (ainda correto: nenhum
  //     deles consegue ver os próprios draws uns dos outros, já que a saída de render-layer do
  //     próprio RmlUi nunca alcança o FBO 0 até o EndFrame(), muito depois de todo elemento
  //     ripple já ter desenhado).
  //
  ...
  //     Gl3RenderInterface::arm_backdrop (render_gl3.cpp), itself called from
  //     RenderGl3::begin_frame_compose, BEFORE Context::Render() runs. Just remembers the
  ...
  //     de Gl3RenderInterface::arm_backdrop (render_gl3.cpp), ele próprio chamado de
  //     RenderGl3::begin_frame_compose, ANTES de Context::Render() rodar. Só lembra o
  ...
  void arm_backdrop_capture(int offset_x, int offset_y, int w, int h) override {
...
private:
  // EN: The real, on-demand capture -- called from render_shader's "glintfx-ripple" branch
  //     (above), never from arm_backdrop_capture directly (see this section's own doc comment
  //     for the full derivation of why). Idempotent within a single frame via
  //     `backdrop_captured_this_frame_` (armed/reset by arm_backdrop_capture, above).
  // PT: A captura real, sob demanda -- chamada do ramo "glintfx-ripple" de render_shader (acima),
  //     nunca diretamente de arm_backdrop_capture (ver o doc-comment desta seção pra derivação
  //     completa do porquê). Idempotente dentro de um único frame via
  //     `backdrop_captured_this_frame_` (armado/resetado por arm_backdrop_capture, acima).
  ...
  //     locations are looked up ONCE here (not per-render_shader-call) since they never change
  ...
  //     locations de uniform são buscadas UMA VEZ aqui (não por-chamada-de-render_shader) já que
  ...
  //     stay at 0 for the entire lifetime of a Gl3FxHook that never renders a
  ...
  //     ficam em 0 pela vida inteira de um Gl3FxHook que nunca renderiza um elemento
  //     "ripple()" -- ver a derivação de custo-zero-quando-inativo de EnsureBackdropCaptured.
  GLuint backdrop_capture_tex_ = 0;
  int cap_w_ = 0;
  int cap_h_ = 0;

  // EN: L1.22-CAPTURE COLD-START FIX — the "armed" capture rectangle (set by
  //     arm_backdrop_capture, read by EnsureBackdropCaptured) and the once-per-frame latch. See
  //     the arm_backdrop_capture/EnsureBackdropCaptured doc comment above for the full
  //     mechanism.
  // PT: FIX DE COLD-START DO L1.22-CAPTURE — o retângulo de captura "armado" (setado por
  //     arm_backdrop_capture, lido por EnsureBackdropCaptured) e a trava de uma-vez-por-frame.
  //     Ver o doc-comment de arm_backdrop_capture/EnsureBackdropCaptured acima pro mecanismo
  //     completo.
...
// EN: FX-CARVE-1 -- the only definition of the factory fx_hook.hpp declares. render_gl3.cpp's
//     Gl3RenderInterface constructor calls this exactly once (under `#if GLINTFX_MODULE_FX`) to
//     construct fx_.
// PT: FX-CARVE-1 -- a única definição da factory que fx_hook.hpp declara. O construtor de
//     Gl3RenderInterface de render_gl3.cpp chama isto exatamente uma vez (sob
//     `#if GLINTFX_MODULE_FX`) pra construir fx_.
std::unique_ptr<FxHook> CreateGl3FxHook() {
  return std::make_unique<Gl3FxHook>();
}
