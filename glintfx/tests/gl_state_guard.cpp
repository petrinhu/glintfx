...
//     Sets a non-default GL state (viewport, scissor, blend, VAO, program, color mask, FBO),
//     snapshots it, calls render(), then asserts each post-render value equals pre-render.
//     A single diverging value is printed and the test fails.
// PT: Verifica que UiLayer::render() não altera nenhuma variável de estado GL capturada.
//     Define estado GL não-default (viewport, scissor, blend, VAO, programa, color mask, FBO),
//     faz snapshot, chama render(), e afirma que cada valor pós-render iguala o pré-render.
//     Um único valor divergente é impresso e o teste falha.
