...
//
//     TST-L1-GLLEAK / GLLEAK-1 ADDITION: 4 more slots the guard captures but no existing test
//     asserted. (1) blend FUNC (glBlendFuncSeparate factors, not just the equation) — the most
//     valuable of the four, since RmlUi's BeginFrame() also calls
//     glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA) every frame, so a broken func-restore
//     corrupts the host's blend state exactly like the equation bug this test already guards
//     against. (2) GL_CULL_FACE enable. (3) GL_STENCIL_TEST enable. (4) GL_READ_BUFFER — sentinel
//     is glReadBuffer(GL_NONE) on the already-bound host FBO (a *user* FBO's default read-buffer
//     is GL_COLOR_ATTACHMENT0, so GL_NONE is a genuine non-default value distinguishable from
//     both the default and from whatever EnsureBackdropCaptured's glReadBuffer(GL_BACK) sets —
//     GL_BACK is invalid on a user FBO under core profile, so it cannot double as the sentinel
//     here).
...
//
//     ADIÇÃO TST-L1-GLLEAK / GLLEAK-1: mais 4 slots que o guard captura mas nenhum teste
//     existente afirmava. (1) blend FUNC (fatores de glBlendFuncSeparate, não só a equação) — o
//     mais valioso dos quatro, já que o BeginFrame() do RmlUi também chama
//     glBlendFunc(GL_ONE, GL_ONE_MINUS_SRC_ALPHA) todo frame, então um restore de func quebrado
//     corrompe o blend do host exatamente como o bug de equação que este teste já guarda contra.
//     (2) habilitação de GL_CULL_FACE. (3) habilitação de GL_STENCIL_TEST. (4) GL_READ_BUFFER —
//     a sentinela é glReadBuffer(GL_NONE) no FBO do host já bound (o read-buffer default de um FBO
//     *de usuário* é GL_COLOR_ATTACHMENT0, então GL_NONE é um valor genuinamente não-default e
//     distinguível tanto do default quanto do que glReadBuffer(GL_BACK) de EnsureBackdropCaptured
//     define — GL_BACK é inválido num FBO de usuário sob core profile, então não pode servir de
//     sentinela aqui).
...
int main() {
  ...
  // EN: GLLEAK-1 sentinels — non-default blend FUNC (distinct from both the GL default
  //     (GL_ONE,GL_ZERO,GL_ONE,GL_ZERO) and from RmlUi's own (GL_ONE,GL_ONE_MINUS_SRC_ALPHA)),
  //     GL_CULL_FACE / GL_STENCIL_TEST enabled (default disabled), and GL_READ_BUFFER=GL_NONE
  //     on the already-bound host fbo (default for a user FBO is GL_COLOR_ATTACHMENT0).
  // PT: Sentinelas do GLLEAK-1 — blend FUNC não-default (distinto do default do GL
  //     (GL_ONE,GL_ZERO,GL_ONE,GL_ZERO) e do próprio do RmlUi (GL_ONE,GL_ONE_MINUS_SRC_ALPHA)),
  //     GL_CULL_FACE / GL_STENCIL_TEST habilitados (default desabilitado), e
  //     GL_READ_BUFFER=GL_NONE no fbo do host já bound (default de um FBO de usuário é
  ...
  // EN: GLLEAK-1 pre-snapshot vars — blend func factors, cull/stencil enables, read buffer.
  // PT: Vars de pré-snapshot do GLLEAK-1 — fatores de blend func, enables cull/stencil, read buffer.
  GLint pre_bsrc_rgb, pre_bdst_rgb, pre_bsrc_a, pre_bdst_a;
  GLint pre_cull, pre_stencil;
  GLint pre_read_buffer;
  ...
  glGetIntegerv(GL_BLEND_SRC_RGB, &pre_bsrc_rgb);
  glGetIntegerv(GL_BLEND_DST_RGB, &pre_bdst_rgb);
  glGetIntegerv(GL_BLEND_SRC_ALPHA, &pre_bsrc_a);
  glGetIntegerv(GL_BLEND_DST_ALPHA, &pre_bdst_a);
  pre_cull = (GLint)glIsEnabled(GL_CULL_FACE);
  pre_stencil = (GLint)glIsEnabled(GL_STENCIL_TEST);
  ...
  // EN: GLLEAK-1 post-snapshot vars — mirrors the pre-snapshot block above.
  // PT: Vars de pós-snapshot do GLLEAK-1 — espelha o bloco de pré-snapshot acima.
  GLint post_bsrc_rgb, post_bdst_rgb, post_bsrc_a, post_bdst_a;
  GLint post_cull, post_stencil;
  GLint post_read_buffer;
  ...
  glGetIntegerv(GL_BLEND_SRC_RGB, &post_bsrc_rgb);
  glGetIntegerv(GL_BLEND_DST_RGB, &post_bdst_rgb);
  glGetIntegerv(GL_BLEND_SRC_ALPHA, &post_bsrc_a);
  glGetIntegerv(GL_BLEND_DST_ALPHA, &post_bdst_a);
  post_cull = (GLint)glIsEnabled(GL_CULL_FACE);
  post_stencil = (GLint)glIsEnabled(GL_STENCIL_TEST);
...
#define CHK_B(name, pre, post)                                                     \
  ...
  // EN: GLLEAK-1 — blend func factors (the func RmlUi's BeginFrame() also overwrites every
  //     frame, alongside the equation already checked above), cull/stencil enables, read buffer.
  // PT: GLLEAK-1 — fatores de blend func (o func que o BeginFrame() do RmlUi também sobrescreve
  //     todo frame, junto da equação já checada acima), enables cull/stencil, read buffer.
  CHK_I("GL_BLEND_SRC_RGB", pre_bsrc_rgb, post_bsrc_rgb);
  CHK_I("GL_BLEND_DST_RGB", pre_bdst_rgb, post_bdst_rgb);
  CHK_I("GL_BLEND_SRC_ALPHA", pre_bsrc_a, post_bsrc_a);
  CHK_I("GL_BLEND_DST_ALPHA", pre_bdst_a, post_bdst_a);
  CHK_I("GL_CULL_FACE", pre_cull, post_cull);
  CHK_I("GL_STENCIL_TEST", pre_stencil, post_stencil);
