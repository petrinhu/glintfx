...
int main() {
  // ===========================================================================================
  // EN: ENC-NOTHROW oracle -- forced-allocation-failure via fork()+RLIMIT_AS, calibrated in two
  //     stages (STAGE 1/2 CALIBRATE, STAGE 2/2 SQUEEZE), the SAME technique
  //     `image_decode_hardening_sanity.cpp`'s own Group B uses (see that file's own top comment
  //     for the full rationale and the mutation-testing story that proved a hand-derived fixed
  //     multiplier does NOT work reliably here). `kW`/`kH` chosen so the pixel buffer (and, for
  //     an uncompressed format like BMP, the encoded output too) is comfortably under
  //     `kMaxImageEncodeBytes` (64 MiB, `image_encode.cpp`) while still large enough that a
  //     genuine allocation failure is forceable.
  // PT: Oráculo do ENC-NOTHROW -- falha-de-alocação-forçada via fork()+RLIMIT_AS, calibrada em
  //     dois estágios (ESTÁGIO 1/2 CALIBRAR, ESTÁGIO 2/2 APERTAR), a MESMA técnica que o próprio
  //     Grupo B de `image_decode_hardening_sanity.cpp` usa (ver o próprio comentário de topo
  //     daquele arquivo pro racional completo e a história de teste de mutação que provou que um
  //     multiplicador fixo derivado à mão NÃO funciona de forma confiável aqui). `kW`/`kH`
  //     escolhidos pra que o buffer de pixel (e, pra um formato sem compressão como o BMP, a
  //     própria saída codificada também) fique confortavelmente abaixo de `kMaxImageEncodeBytes`
  //     (64 MiB, `image_encode.cpp`) mas ainda grande o bastante pra que uma falha de alocação
  //     genuína seja forçável.
  // ===========================================================================================
  {
    const int kW = 3000;
    const int kH = 2000;
