...
;     anticipated. Each function receives its args per the System V AMD64 C-ABI (1st..6th
;     integer/pointer arg in rdi,rsi,rdx,rcx,r8,r9; a 7th spills to the stack at [rsp+8]) and
;     reshuffles them into the raw `syscall` instruction's own convention (rax=number, args in
;     rdi,rsi,rdx,r10,r8,r9 -- note r10 replaces rcx, which the `syscall` instruction itself
;     clobbers together with r11). Pure register glue, no branches, no stack frame -- these are
;     leaf functions.
; PT: Implementa syscall1/2/3/6 (ADR-0001). syscall0/4/5 continuam podadas, codigo morto
;     (AUD-C0-2, sem call-site real); syscall2 VOLTA (SOV-ALLOC, W15) -- `sys_munmap`
;     (`munmap(addr, len)`, 2 args crus) e' um chamador novo genuino, exatamente o caso "barato
;     de readicionar" que o ADR-0001 previu. Cada funcao recebe seus args pela C-ABI System V
