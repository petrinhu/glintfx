...
# EN: Builds/refreshes localhost/glintfx-ci:f44, the local image used to reproduce
#     glintfx's heavy checks (ASan/LSan/UBSan, own-font-engine build) on a Fedora 44
...
#     tools/ci/Containerfile.f44 or its package list changes; the docker build cache
#     makes a no-op re-run near-instant.
#
#     Bumped from fedora:42 to fedora:44 (CI-DISTROMATRIX half A, 2026-08-06) -- tag
#     and Containerfile name renamed together, see Containerfile.f44's own header.
#
#     Typical use after building:
#       docker run --rm -v "$PWD":/src -w /src localhost/glintfx-ci:f44 bash -c '<cmd>'
#
# PT: Builda/atualiza a localhost/glintfx-ci:f44, a imagem local usada pra reproduzir
#     À MÃO os checks pesados da glintfx (ASan/LSan/UBSan, build do motor de fonte
#     próprio) sobre uma base Fedora 44. Não existe mais runner de CI nesta máquina --
...
#     `localhost/`. Re-execute sempre que tools/ci/Containerfile.f44 ou sua lista de
#     pacotes mudar; o cache de build do docker deixa uma re-execução no-op quase
#     instantânea.
#
#     Subiu de fedora:42 pra fedora:44 (CI-DISTROMATRIX metade A, 2026-08-06) -- tag e
#     nome do Containerfile renomeados juntos, ver o próprio cabeçalho do
#     Containerfile.f44.
#
...
tag="localhost/glintfx-ci:f44"

echo "==> building ${tag} from tools/ci/Containerfile.f44"
docker build -f "${repo_root}/tools/ci/Containerfile.f44" -t "${tag}" "${repo_root}"
